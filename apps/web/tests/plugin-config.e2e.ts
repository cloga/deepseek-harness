...
describe('web e2e: plugin configuration pages', () => {
  ...
  it('saves limits and the model allowlist together from the shared card', async () => {
    ...
    // Wait for the shared form to observe the limits draft before another
    // section re-renders it; Save must capture both dirty owners.
    ...
    await expect.poll(async () => (await settingsDocument()).includes('maxDepth: 2'), { timeout: 10_000 })
      .toBe(true)
