...
it('keeps the alpha2 Host lifecycle and checks command readiness before reporting the Web URL', () => {
  const source = readFileSync(new URL('../src/index.ts', import.meta.url), 'utf8')
  expect(source).toContain('const application = runProfile(')
  expect(source).toContain('control.commands = installDesktopPluginCommands(ctx,')
  expect(source).toContain('await provideDesktopPackageTransactions(ctx)')
  expect(source).toContain('control.commands?.bridge.receive(message) === true')
  expect(source).toContain("if (message.type === 'shutdown') { void stop(); return }")
  expect(source).toContain("message.type !== 'update-tasks'")
  expect(source.indexOf('control.commands?.bridge.dispose()')).toBeLessThan(source.indexOf('await running?.shutdown.shutdown(0)'))
  expect(source.indexOf('await control.commands.ready()')).toBeLessThan(source.indexOf("type: 'ready', url"))
  expect(source).toContain("await send({ type: 'shutdown-complete' })")
  expect(source).toContain('ctx.webServer.collectIndexInjections()')
  for (const retired of ['runDesktopHost', "'./wire.ts'", 'DESKTOP_REQUEST_PIPE_FD', 'DESKTOP_RESPONSE_PIPE_FD']) {
