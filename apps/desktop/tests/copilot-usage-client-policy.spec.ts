...
const clientSha256 = '6d6a7df36c377b7485b31d45511a8b582f5b745a1030a7f6e4c35a181ad52435'
...
describe('independently reviewed Copilot positive-usage Client policy', () => {
  it('admits the reviewed source only with the original alpha.33 Client digest', () => {
  ...
  it.each([
    ...
    expect(() => { assertReviewedCopilotUsageClient(source, clientSha256) }).toThrow('reviewed Copilot alpha.33 source')
  })
  ...
  it.each([
    ...
    expect(() => { assertReviewedCopilotUsageClient(changed, clientSha256) }).toThrow('reviewed Copilot alpha.33 source')
  })
