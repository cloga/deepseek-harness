...
import { DESKTOP_RUNTIME_FILE, readDesktopRuntime, renderDesktopRuntimeDescriptor, runtimePath, verifyDesktopRuntime } from '../src/runtime-tree.ts'
...
it('preserves production descriptor bytes when the writer delegates to the shared renderer', () => {
  const dsh = join(fixture(), 'dsh')
  const descriptor = readDesktopRuntime(dsh)
  const before = JSON.stringify(descriptor)
  const originalFormat = `${JSON.stringify(descriptor, undefined, 2)}\n`
  expect(readFileSync(join(dsh, DESKTOP_RUNTIME_FILE), 'utf8')).toBe(originalFormat)
  expect(renderDesktopRuntimeDescriptor(descriptor)).toBe(originalFormat)
