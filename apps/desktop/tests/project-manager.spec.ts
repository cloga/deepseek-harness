...
describe('desktop external plugin profile', () => {
  it('does not recreate or clean a missing active profile while an activation journal owns recovery', async () => {
    ...
    const transaction = join(dirname(manager.paths.profile), `.desktop.package-stage-${transactionId}`)
    const rollback = join(transaction, 'rollback')
    mkdirSync(rollback, { recursive: true })
    writeFileSync(join(rollback, 'user-data'), 'preserve')
    writeFileSync(join(transaction, 'ACTIVATION.json'), JSON.stringify({ schemaVersion: 1, transactionId,
      ownerFingerprint: 'a'.repeat(64), baseFingerprint: 'b'.repeat(64), baseGraphFingerprint: 'c'.repeat(64),
      candidateFingerprint: 'd'.repeat(64), intentFingerprint: 'e'.repeat(64), phase: 'swapping' }))
    await expect(manager.applyRelease()).rejects.toThrow('unfinished package activation')
    await expect(manager.disableAllPlugins()).rejects.toThrow('unfinished package activation')
    expect(existsSync(manager.paths.profile)).toBe(false)
    expect(readFileSync(join(rollback, 'user-data'), 'utf8')).toBe('preserve')
  })

  it('grants fresh-profile authority only after exclusive successful initialization', async () => {
    const { manager } = setup()
    expect(manager.createdProfile).toBe(false)
    await manager.applyRelease()
    expect(manager.createdProfile).toBe(true)
    await manager.applyRelease()
    expect(manager.createdProfile).toBe(false)
  })

  it('does not infer fresh-profile authority from a pre-existing empty directory', async () => {
    const { manager } = setup()
  ...
  it('does not retain fresh-profile authority after failed initialization', async () => {
    const { manager } = setup()
    await manager.applyRelease()
    expect(manager.createdProfile).toBe(true)
  ...
  it('needs no runtime or package manifest when no plugins have been installed', async () => {
    ...
    expect(existsSync(lockPath(manager))).toBe(false)
  })
  ...
  it('rejects linked activation targets and migrates only generated pnpm defaults in real profiles', async () => {
    ...
    await expect(manager.applyRelease()).rejects.toThrow('real directory')
    expect(readdirSync(target)).toEqual([])
    unlinkSync(manager.paths.profile)
    await manager.applyRelease()
    const path = join(manager.paths.profile, 'pnpm-workspace.yaml')
...
describe.each(['applyRelease', 'disableAllPlugins'] as const)('desktop profile lock during %s', (operation) => {
  beforeEach(() => { vi.spyOn(Date, 'now').mockReturnValueOnce(0).mockReturnValue(120001) })
  afterEach(() => { vi.restoreAllMocks() })
  it('preserves a live owner lock and leaves the profile untouched', async () => {
    const { manager } = setup()
    mkdirSync(manager.paths.profile, { recursive: true })
    const owner = `${String(process.pid)}\n`
    writeFileSync(lockPath(manager), owner)
    await expect(manager[operation]()).rejects.toThrow('timed out waiting for the writer lock')
    expect(readFileSync(lockPath(manager), 'utf8')).toBe(owner)
    expect(readdirSync(manager.paths.profile)).toEqual([])
  })

  it('does not guess ownership or remove an orphaned lock automatically', async () => {
    const { manager } = setup()
    mkdirSync(manager.paths.profile, { recursive: true })
    writeFileSync(lockPath(manager), '99999999\n')
    const probe = vi.spyOn(process, 'kill')
    try {
      await expect(manager[operation]()).rejects.toThrow('timed out waiting for the writer lock')
      expect(probe).not.toHaveBeenCalled()
      expect(readFileSync(lockPath(manager), 'utf8')).toBe('99999999\n')
  ...
  it('rejects a directory at the lock path', async () => {
    const { manager } = setup()
    mkdirSync(lockPath(manager), { recursive: true })
    await expect(manager[operation]()).rejects.toThrow('timed out waiting for the writer lock')
    expect(lstatSync(lockPath(manager)).isDirectory()).toBe(true)
  })
  ...
  it('rejects a linked lock without touching its target', async () => {
    ...
    symlinkSync(target, lockPath(manager), process.platform === 'win32' ? 'junction' : 'dir')
    try {
      await expect(manager[operation]()).rejects.toThrow('timed out waiting for the writer lock')
      expect(lstatSync(lockPath(manager)).isSymbolicLink()).toBe(true)
      expect(readFileSync(join(target, 'sentinel'), 'utf8')).toBe('retain')
    } finally {
      unlinkSync(lockPath(manager))
    }
  })
})
