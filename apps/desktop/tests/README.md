...
Cleanup invokes the validated owned executable with fixed `/currentuser /S` arguments matching `QuietUninstallString`, never by executing registry-supplied command text. A post-uninstall failure retains bounded scalar observations of launcher state, remaining files, registration and process state; these observations do not establish the cause of a timeout. Exit/removal deadlines, cleanup assertions and primary-failure preservation remain unchanged. The pure registration fixtures exercise this naming rule without reading the registry or starting an uninstaller; hosted installed-upgrade acceptance remains separate.
