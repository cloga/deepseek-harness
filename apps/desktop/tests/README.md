...
[Packaged Copilot acceptance](fixtures/copilot-release-smoke.ts) combines the complete initial/restart Copilot alpha.32 assertions and an optional cleanup-observer canary in one provisioned profile. Only after those assertions pass does the observer run once against the real profile. The deliberate marker is recognized by exact error identity and contained; other failures cannot count as that expected canary. The normal `acceptance.json` receipt accompanies separate `observer-cleanup.json` evidence recording combined success and removal of the owned home and ancestor canary. The workflow uses this combined run rather than provisioning a second fresh profile from GitHub. This evidence does not establish unexpected-failure receipt withholding, live quota access, OAuth, model calls or search.
