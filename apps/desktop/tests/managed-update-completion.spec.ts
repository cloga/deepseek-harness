...
async function completionFixture(sequence = 2, reconcile = completeDesktopManagedUpdate) {
  ...
  await writeFile(join(profile, 'desktop-plugin-provisioning-state.json'), JSON.stringify({
    schemaVersion: 2, capability: DESKTOP_NATIVE_PLUGIN_PROVISIONING_CAPABILITY, planSchemaVersion: plan.schemaVersion,
...
const inventories = [
  'valid', 'missing-state', 'required-valid', 'required-override-valid', 'required-artifact-drift',
...
it.each(inventories)('requires active inventory before completion: %s', async (inventory) => {
  ...
  const overrideInventory = inventory === 'required-override-valid'
  const provisioning = parseDesktopPluginProvisioningPlan({
    schemaVersion: overrideInventory ? 2 : 1, mode: 'exact',
    plugins: required ? [{
      required: true,
      ...(overrideInventory ? { sourcePolicy: 'compatible-user-override' } : {}),
      source: {
  ...
  if (required) {
    ...
    const effectiveArtifact = overrideInventory ? Buffer.from('compatible completion override') : artifact
    const effectiveSource = overrideInventory ? {
      ...source, tag: 'v0.9.0', asset: 'fixture-plugin-0.9.0.tgz', assetId: 3, version: '0.9.0',
      size: effectiveArtifact.length, sha256: sha256(effectiveArtifact),
      checksumManifest: {
        ...source.checksumManifest, assetId: 4,
        url: 'https://github.com/cloga/fixture-plugin/releases/download/v0.9.0/SHA256SUMS',
      },
    } : source
    const receipt = {
      schemaVersion: 1, capability: DESKTOP_NATIVE_VERIFIED_RELEASE_CAPABILITY,
      source: effectiveSource, releaseId: 1, assetId: effectiveSource.assetId,
      packageName: effectiveSource.packageName, version: effectiveSource.version, artifactSha256: effectiveSource.sha256,
    ...
    await writeFile(join(artifactDirectory, `${effectiveSource.sha256}.tgz`),
      inventory === 'required-artifact-drift' ? Buffer.from('changed artifact') : effectiveArtifact)
    ...
    profileManifest.dependencies[source.packageName] = `file:.desktop-plugin-artifacts/${effectiveSource.sha256}.tgz`
    ...
    await writeFile(join(plugin, 'package.json'), JSON.stringify({
      name: source.packageName, version: effectiveSource.version, dsh: { bundle: { patch: 'cordis.patch.yml' } },
    }))
    await writeFile(join(plugin, 'cordis.patch.yml'), '[]\n')
    results.push({
      name: source.packageName, required: true, status: 'active', requestedSource: source,
      sourcePolicy: overrideInventory ? 'compatible-user-override' : 'strict-pin',
      effective: overrideInventory ? 'user-override' : 'plan', effectiveSource, receipt,
    })
  }
  if (inventory !== 'missing-state') {
    await writeFile(join(profile, 'desktop-plugin-provisioning-state.json'), JSON.stringify({
      schemaVersion: 2, capability: DESKTOP_NATIVE_PLUGIN_PROVISIONING_CAPABILITY,
      planSchemaVersion: provisioning.schemaVersion,
  ...
  if (inventory === 'valid' || inventory === 'required-valid' || inventory === 'required-override-valid' || inventory === 'required-first-install') {
