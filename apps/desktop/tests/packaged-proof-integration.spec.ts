...
function integrationFixture(alteredClientBytes?: string) {
  ...
  for (const path of [join(resources, 'desktop-provisioning'), join(resources, 'managed-update'), releaseAssets, packagedEvidence, baselineDirectory, evidence]) mkdirSync(path, { recursive: true })
  // Synthetic prerequisite only: this integration never invokes the helper or claims its real bootstrap was exercised.
  save(join(packagedEvidence, 'helper-acceptance.json'), {
