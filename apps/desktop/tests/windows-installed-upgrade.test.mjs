...
test('installed main PID remains authority when Playwright launcher differs', () => {
  ...
  assert.ok(driver.includes('Assert-BaselineProcessBinding $live $baselineIdentity.ExecutableSha256'))
  assert.doesNotMatch(driver, /if \(\$live\.Path -ne \$application\)/u)
...
`)
  ...
  assert.equal(cases.match.literalPathMatch, true)
  ...
  assert.equal(cases.mismatch.literalPathMatch, false)
