...
读取原生可访问性之前，Windows PowerShell 辅助程序注册已安装框架的标准客户端 UI Automation provider。打包前的 Node 检查在自有屏幕外 Win32 Edit／Button 控件上执行实际初始化器，要求预期的 Value／Invoke 模式，并在不调用控件的情况下拒绝外部归属。这验证该隔离辅助程序的准备，不证明历史失败原因或完整原生选择器通过。严格的控件类型、ID 与进程检查保持不变。Fixture 在进程内声明固定 Win32 调用来创建自有控件，不再额外编译 fixture C#；产品初始化器及其自身编译保持不变，且仅在省略 provider 的负例观察之后运行。固定的 stderr 阶段标签在同一 15 秒子进程预算内，标记程序集加载、互操作声明、自有控件创建、观察、初始化和清理的前后阶段。最后观察到的标签仅把超时缩小到一个区间；没有输出不证明启动卡住，新增诊断 I/O 也不确立先前失败的原因。任务限定的源码诊断 workflow 只运行两个既有安装器前置测试套件，不构建或启动 Desktop；其源码绑定日志属于诊断证据，不是打包验收。
