...
读取原生可访问性之前，Windows PowerShell 辅助程序注册已安装框架的标准客户端 UI Automation provider。打包前的 Node 检查在自有屏幕外 Win32 Edit／Button 控件上执行实际初始化器，要求预期的 Value／Invoke 模式，并在不调用控件的情况下拒绝外部归属。这验证该隔离辅助程序的准备，不证明历史失败原因或完整原生选择器通过。严格的控件类型、ID 与进程检查保持不变。Fixture 在进程内声明固定 Win32 调用来创建自有控件，不再额外编译 fixture C#；产品初始化器及其自身编译保持不变，且仅在省略 provider 的负例观察之后运行。固定的 stderr 阶段标签在同一 15 秒子进程预算内，标记程序集加载、互操作声明、自有控件创建、观察、初始化和清理的前后阶段。最后观察到的标签仅把超时缩小到一个区间；没有输出不证明启动卡住，新增诊断 I/O 也不确立先前失败的原因。发布前置检查和任务限定的源码诊断使用 `--test-concurrency=1` 运行相同的两个安装器前置测试文件，保留独立 Node 进程及每个既有子进程期限。该设置避免文件级原生编译和桌面资源操作重叠；它既不证明历史超时原因，也不刻意预热 provider。诊断 workflow 不构建或启动 Desktop，其源码绑定日志不构成打包验收。

包操作中的模型选择等待选中标题及精确提供方菜单内已选中的选项，再关闭菜单并记录就绪。新 Session 必须独立显示预期模型，之后才检查草稿／附件。仅有可见 composer 或点击完成，不能证明异步选择或继承默认状态；fixture 不发送模型请求。

每个自有 Electron Page 拥有独立错误收集；所有代际快照都保留在最终汇总中。同一 Page 的 Host 替换仍位于观察区间内。正常关闭先等待最后一次原生观察，再封存不可变快照、仅移除自有监听器，并在主动 Exit 前要求累计错误为零；原生退出与进程检查仍是独立义务。失败清理在关闭 Page 前封存，移除失败时保留主要错误并继续其他自有清理。在范围内的错误不能被后续启动清除，封存后的回调不能修改已接受记录。惰性观察器测试不替代真实打包交互。
...
原生 composer 阶段紧接 `restart:closed`，依次记录精确的 seeded、launch、application、observed 与 closed 事件；仅当实际进行了公开选择时，才允许在 application 之后立即记录 `native-composer:provider-deferred`。只有应用 URL 不代表 Client 就绪：所有者在自有错误观察器内部、一次性引导处理器安装之后，使用原有 120 秒启动预算等待公开的设置按钮，再进行侧栏导航。独立的 15 秒导航时限保持不变。原生控件通过唯一公开 composer dock 内精确的可访问按钮名称定位，而不是依赖已退役的原生组件标记；原子测量要求同一组控件，并拒绝歧义匹配。实际 StatsPills 组件回归保留无计时数据时不可交互的读数，而无布局测试中的矩形不构成打包几何证据。打包 Session 写入使用独占归属标记，只有句柄与 context 关闭后才报告成功；其有界 stdout 不是新增公开资产。临时 `native-composer-geometry.json` 包含完整共享源码／运行／产物身份、经过评审的插件来源及 Client hash、合成持久化历史／登出额度 scope、按顺序排列的 1280／400 几何、原生与 Copilot 对话框观察，以及空的检查错误列表。验证器要求它与嵌入的功能对象精确相等，符合封闭有限几何和实际原生对齐／样式规则，并通过 `inputs['packaged.nativeComposer']` 绑定原始字节。监听器移除会尝试两个注册，且不替换主要失败；错误在主动关闭前封存，而不是在检查期间被静默忽略。Session 写入、启动、检查、receipt、监听器、写文件、关闭或清理失败都会阻止完成。`combined-suite-v3` 是显式消费格式；历史 v1/v2 记录仍保持区别。
