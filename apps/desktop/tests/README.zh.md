...
读取原生可访问性之前，Windows PowerShell 辅助程序注册已安装框架的标准客户端 UI Automation provider。打包前的 Node 检查在自有屏幕外 Win32 Edit／Button 控件上执行实际初始化器，要求预期的 Value／Invoke 模式，并在不调用控件的情况下拒绝外部归属。这验证该隔离辅助程序的准备，不证明历史失败原因或完整原生选择器通过。严格的控件类型、ID 与进程检查保持不变。
