...
function Write-UninstallFailureDiagnostics($UninstallerProcess, $Errors, $Copy = $null) {
    $observation = [ordered]@{
        ...
        workerWindowCount = $null; workerVisibleWindowCount = $null; workerUnresponsiveWindowCount = $null; workerWindowsTruncated = $null
    ...
    if ($observation.launcherExited -eq $false) {
        try {
            # DiagnosticText bounds owned-window reads to 64 controls/two seconds; discard all text, paths and window handles.
            $windowText = [InstallerCapture]::DiagnosticText($UninstallerProcess.Id)
            $windows = @($windowText -split '\r?\n' | Where-Object { $_.StartsWith('HWND=') })
            $observation.workerWindowCount = $windows.Count
            $observation.workerVisibleWindowCount = @($windows | Where-Object { $_ -match ' VISIBLE=True ' }).Count
            $observation.workerUnresponsiveWindowCount = @($windows | Where-Object { $_ -match ' TEXT=<unresponsive>$' }).Count
            $observation.workerWindowsTruncated = $windowText -match 'LIMIT_REACHED=True'
        } catch { $observationErrors.Add('worker-window-state-unavailable') }
    }
    try {
