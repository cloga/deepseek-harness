...
function fixture(temporaryRoot = tmpdir()) {
  ...
  for (const path of [releaseAssets, ordinaryEvidence, packagedEvidence, baselineDirectory, evidence]) mkdirSync(path, { recursive: true })
  ...
  save(join(ordinaryEvidence, 'helper-acceptance.json'), { helperSha256: sha('synthetic helper'), isolatedBootstrap: 'passed', validSyntheticHandoffAcknowledged: true,
...
function mutatePositive(callback: (value: Json) => void): void {
  ...
  current.edit(join(current.options.ordinaryEvidence, 'positive-usage.json'), (value) => { value.cases = positive.cases })
  current.edit(join(current.options.ordinaryEvidence, 'acceptance.json'), (value) => { value.positiveCopilotUsage = positive.cases })
  current.seal()
}
...
describe('positive usage evidence in both original directories', () => {
  ...
  for (const index of [0, 1]) {
    it.each(positiveFlags)(`rejects false or missing %s in original directory ${index}`, (field) => {
      const file = join(roots()[index]!, 'positive-usage.json')
      mutate(file, (value) => { object(array(value.cases)[1])[field] = false })
      expect(verify).toThrow()
      mutate(file, (value) => { Reflect.deleteProperty(object(array(value.cases)[1]), field) })
      expect(verify).toThrow()
    })
    it.each([2, 3])(`rejects legacy quota count %i in original directory ${index}`, (quotaReads) => {
    ...
    it(`requires schema2 and precise optional deferred-event placement in directory ${index}`, () => {
      ...
      for (const damage of ['valid', 'misplaced', 'duplicate']) {
        mutate(file, (value) => {
          ...
          const where = events.indexOf(damage === 'misplaced' ? 'initial:account' : 'initial:application') + 1
          events.splice(where, 0, 'initial:provider-deferred')
          if (damage === 'duplicate') events.splice(where, 0, 'initial:provider-deferred')
          value.timeline = events.map((event, milliseconds) => ({ event, milliseconds }))
        })
        if (damage === 'valid') expect(verify().packagedFunctionalVerified).toBe(true)
  ...
  it.each(['scope', 'provider', 'usageText', 'quotaReads', 'selectorErrors', 'forbiddenRemoteCalls', 'hostTransport', ...positiveFlags])('rejects missing case field %s even in matching receipts', (field) => {
    casesInAllReceipts((cases) => { Reflect.deleteProperty(object(cases[0]), field) })
    expect(verify).toThrow()
  })
  it.each(['scope', 'provider', 'usageText', 'quotaReads', 'selectorErrors', 'forbiddenRemoteCalls', 'hostTransport', ...positiveFlags])('rejects invalid case field %s even in matching receipts', (field) => {
...
describe('CI-only fork qualification from retained evidence', () => {
  ...
  it('accepts the complete synthetic graph, preserves all input bytes, and retains narrow scope', () => {
    ...
    expect(summary).toMatchObject({ sourceCommit: source, sourceTree: tree, runId: '123', runAttempt: '2', packagedFunctionalVerified: true, actualInstalledUpgradeVerified: true, normalPackagedAcceptanceCompleted: true, canaryNormalAcceptanceCompleted: false,
      limits: {
  ...
  it('hashes independent ordinary acceptance without requiring the canary evidence UUID', () => {
    const ordinary = join(current.options.ordinaryEvidence, 'acceptance.json')
    expect(json(ordinary).evidenceId).not.toBe(json(packaged('functional-results.json')).evidenceId)
    expect(verify().inputs['ordinary.acceptance']).toBe(rawHash(ordinary))
  })
  it.each(['sourceCommit', 'sourceTree', 'runId', 'runAttempt', 'planSha256', 'runtimeSha256', 'executableSha256', 'provisioningSha256', 'capabilitySha256',
    'evidenceId', 'scope', 'functionalAssertionsCompleted', 'normalAcceptanceCompleted', 'cleanupVerified', ...packagedTrue, ...packagedFalse])('rejects mismatched ordinary %s', (field) => {
    mutate(join(current.options.ordinaryEvidence, 'acceptance.json'), (value) => { value[field] = typeof value[field] === 'boolean' ? !value[field] : 'foreign' })
    expect(verify).toThrow()
  })
  it('cannot use a helper copy in the canary directory instead of the original', () => {
    const path = join(current.options.ordinaryEvidence, 'helper-acceptance.json')
    writeFileSync(packaged('helper-acceptance.json'), readFileSync(path)); rmSync(path)
    expect(verify).toThrow()
  })
  it('requires ordinary acceptance independently of the canary suite', () => {
  ...
  it('binds helper bootstrap/ACK/cancel to finalized helper bytes without claiming live handoff', () => {
    const summary = verify()
    expect(summary.inputs['ordinary.helper']).toBe(rawHash(join(current.options.ordinaryEvidence, 'helper-acceptance.json')))
  ...
  it('requires original helper acceptance, not only helperStandalone build validation', () => {
    rmSync(join(current.options.ordinaryEvidence, 'helper-acceptance.json')); expect(verify).toThrow()
  })
  it.each(['helperSha256', 'isolatedBootstrap', 'validSyntheticHandoffAcknowledged', 'cancellationCompleted', 'nodePath', 'nodeOptions', 'manifestTransport', 'liveHandoff', 'installerStarted'])('rejects changed helper %s', (field) => {
    mutate(join(current.options.ordinaryEvidence, 'helper-acceptance.json'), (value) => { value[field] = typeof value[field] === 'boolean' ? !value[field] : 'foreign' }); expect(verify).toThrow()
  })
  it('rejects additional helper authority fields', () => {
    mutate(join(current.options.ordinaryEvidence, 'helper-acceptance.json'), (value) => { value.liveUpgradeVerified = true }); expect(verify).toThrow()
  })
...
describe('fixed qualification CLI', () => {
  function cli(change: Record<string, string> = {}) {
    ...
    const args = { plan: join(root, 'apps/desktop/release/cloga-windows-x64.json'), 'release-assets': join(root, 'dist/desktop-fork-release'), 'ordinary-evidence': join(root, 'dist/desktop-copilot-acceptance'), 'packaged-evidence': join(root, 'dist/desktop-copilot-observer-canary'),
  ...
  it.each(['plan', 'release-assets', 'ordinary-evidence', 'packaged-evidence', 'upgrade-root', 'baseline-directory', 'output'])('rejects noncanonical --%s', (name) => { cli({ [name]: join(current.directory, 'foreign') }); expect(runForkQualificationCli).toThrow(/path differs/u) })
