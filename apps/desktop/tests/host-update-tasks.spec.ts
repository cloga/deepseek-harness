...
/** Join the real root invariant startup even when the task-service injection stays pending. */
async function awaitStartup(context: Context) {
  // The official test setup joins invariant readiness to ordinary plugin thenables.
  return await context.plugin(function desktopUpdateTestReady() {})
}
...
beforeEach(async () => {
  ...
  ctx.provide('jobs', { list: (agent?: AgentState) => jobs.get(agent) ?? [], onJobsChanged: () => () => {} } as unknown as Context['jobs'])
  inspect = installDesktopUpdateTaskControl(ctx)
  await awaitStartup(ctx)
})
...
describe('Desktop Host update task protection', () => {
  it('installs a replacement generation barrier before task services or profile APIs exist', async () => {
    await ctx.fiber.dispose()
    ctx = new Context()
    inspect = installDesktopUpdateTaskControl(ctx, true)
    await awaitStartup(ctx)
    const next = vi.fn(async () => {})
    const early = request(next)
    await early.done
    expect(early.response.writeHead).toHaveBeenCalledWith(503)
  ...
  it('joins root readiness before disposal while task services are still unavailable', async () => {
    const empty = new Context()
    try {
      const unavailable = installDesktopUpdateTaskControl(empty, true)
      const ready = await awaitStartup(empty)
      expect(ready.state).toBe(FiberState.ACTIVE)
      expect(empty.get('agents')).toBeUndefined()
      expect(empty.get('jobs')).toBeUndefined()
      await expect(unavailable('inspect')).rejects.toThrow('task services are unavailable')

      await empty.fiber.dispose()
      expect(ready.state).toBe(FiberState.DISPOSED)
      await expect(unavailable('inspect')).rejects.toThrow('Host is stopping')
