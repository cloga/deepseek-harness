...
it.each(['deadline', 'user', 'user-named-timeout'] as const)('keeps %s cancellation distinct after the real owned child exits', { timeout: 60000 }, async mode => {
  ...
  const watcher = watch(root, (_event, name) => { if (String(name) === 'ready') ready.resolve() })
  watcher.on('error', ready.reject)
  const controller = new AbortController()
  let run: ReturnType<typeof runDesktopPackagePnpm> | undefined
  ...
  run = runDesktopPackagePnpm({ node: process.execPath, pnpm: stub, nodeBin: dirname(process.execPath) }, {
