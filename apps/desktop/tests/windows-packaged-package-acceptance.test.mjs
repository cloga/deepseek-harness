...
test('installed driver runs the separately scoped case before uninstall and retains unverified installer flags', () => {
  ...
  assert.ok(driver.indexOf('Start-Fixture package') < driver.indexOf("Start-Owned $uninstaller '/currentuser /S'"))
  ...
  assert.ok(driver.indexOf('$packageCleanup.cleanupVerified') < driver.indexOf("Start-Owned $uninstaller '/currentuser /S'"))
