  ...
  it('keeps the explicit unsigned fork capability channel separate from official signing and policy files', async () => {
    await withDirectory(async directory => {
      const fork = { DSH_DESKTOP_APP_ID: 'io.github.cloga.deepseek-harness.desktop',
        ...
        DSH_DESKTOP_PLUGIN_PROVISIONING_PLAN: join(directory, 'plan.json'),
        DSH_DESKTOP_WINDOWS_TOKEN_PIN: 'fixture-pin', DSH_DESKTOP_RELEASE_GITHUB_TOKEN: 'fixture-release-token',
        DSH_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN: 'https://unrelated-official-policy.example', PATH: 'build-tools' }
      const selected = loadDesktopPackageEnvironment('win32', fork, directory, { unsigned: true, arch: 'x64' })
      expect(selected.DSH_DESKTOP_APP_ID).toBe(fork.DSH_DESKTOP_APP_ID)
      expect(selected.DSH_DESKTOP_MANAGED_UPDATE_CAPABILITY).toBe(fork.DSH_DESKTOP_MANAGED_UPDATE_CAPABILITY)
