...
interface ReadyEvent {
  ...
  readonly packages?: readonly ProfilePackageHealth[] | undefined
}
...
function isDesktopHostEvent(message: unknown): message is DesktopHostEvent {
  ...
  switch (candidate.type) {
    case 'shutdown-complete':
      return true
    case 'ready':
      return typeof candidate.url === 'string' && (candidate.packages === undefined
        || (Array.isArray(candidate.packages) && candidate.packages.length <= 4096 && candidate.packages.every((item: unknown) => {
          if (typeof item !== 'object' || item === null || Array.isArray(item)) return false
          const entry = item as Record<string, unknown>
          return Object.keys(entry).every(key => ['name', 'version', 'enabled', 'healthy'].includes(key))
            && typeof entry.name === 'string' && entry.name.length <= 214
            && (entry.version === undefined || (typeof entry.version === 'string' && entry.version.length <= 128))
            && typeof entry.enabled === 'boolean' && typeof entry.healthy === 'boolean'
        })))
    case 'fatal':
...
export class DesktopHostProcess {
  ...
  private packageCleanupFailure: Error | undefined
  ...
  async start(): Promise<DesktopHostReady> {
    ...
    const packages = this.packageTransactions === undefined ? undefined : new DesktopPackageTransactionIpc(this.packageTransactions)
    const child = spawn(this.node, [
      '--expose-internals',
      '--import', pathToFileURL(join(this.runtimeDir, 'node_modules', '@deepseek-ai', 'dsh-desktop-host', 'register-module-resolution-policy.mjs')).href,
      ...
      env: desktopNodeEnvironment(this.node, undefined, {
        ...this.environment, DSH_DESKTOP_STAGED_PACKAGES: packages === undefined ? '0' : '1',
        DSH_DESKTOP_ADMISSION_LOCKED: this.admissionLocked ? '1' : '0',
      }),
    ...
    child.on('message', (message: unknown) => {
      ...
      if (message.type === 'ready') this.readyResolve({ url: message.url, injections: message.injections, packages: message.packages })
    ...
    this.exitPromise = new Promise<void>((resolve) => {
      child.once('close', (code) => {
        ...
        void Promise.resolve(packages?.dispose()).then(resolve, (error: unknown) => {
          this.packageCleanupFailure = error instanceof Error ? error : new Error(String(error))
          this.fail(this.packageCleanupFailure)
          resolve()
        })
      })
    })
  ...
  async stop(requireGraceful = false): Promise<void> {
    ...
    if (requireGraceful && (!graceful || child.exitCode !== 0 || !this.shutdownCompleted || this.packageCleanupFailure !== undefined)) {
      // This diagnostic reaches expandable UI; arbitrary plugin stderr can contain credentials.
      throw new DesktopHostUncleanExitError(`desktop update: Host did not complete graceful task teardown (exit ${String(child.exitCode)}, signal ${String(child.signalCode)}, shutdown acknowledged ${String(this.shutdownCompleted)}, graceful deadline exceeded ${String(!graceful)}, package cleanup failed ${String(this.packageCleanupFailure !== undefined)})`)
    }
  }
