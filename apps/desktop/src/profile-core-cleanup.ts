 ...
 * Refuse before writes when cleanup would invalidate a lockfile retained by external dependency roots,
 * or when a missing manifest prevents establishing those roots. This cleanup does not migrate locked plugins.
...
export function cleanProfileCorePackages(profile: string, packageNames: readonly string[], production: boolean): void {
  ...
  let retainedDependencies = false
  if (manifest !== undefined) {
    for (const field of ['dependencies', 'devDependencies', 'optionalDependencies', 'peerDependencies']) {
      manifestChanged = prune(manifest, field, names) || manifestChanged
      if (manifest[field] !== undefined && Object.keys(object(manifest[field])).length > 0) retainedDependencies = true
    }
  ...
  // Inspect every removal candidate before touching the active profile, even after finding residue.
  const packageResidue = roots.flatMap(root => [...names].map(name => join(root, name)))
    .filter(path => stat(path) !== undefined)
  const lockPath = join(profile, 'pnpm-lock.yaml')
  const lock = stat(lockPath)
  if (!manifestChanged && !workspaceChanged && packageResidue.length === 0) return
  if (lock !== undefined && (retainedDependencies || manifest === undefined)) {
    throw new Error('desktop profile cleanup: cannot remove core residues with an existing pnpm-lock.yaml and retained or unknown external dependency roots; a separately qualified lock-preserving migration is required')
  }
  const manifestText = manifestChanged ? `${JSON.stringify(manifest, undefined, 2)}\n` : undefined
  const workspaceText = workspaceChanged ? dump(workspace) : undefined
  if (lock !== undefined) remove(lockPath)
  if (manifestText !== undefined) writeFileSync(manifestPath, manifestText)
  if (workspaceText !== undefined) writeFileSync(workspacePath, workspaceText)
  for (const path of packageResidue) remove(path)
}
