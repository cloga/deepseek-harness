...
import {
  ...
  type ProfilePackageTransactions, type ProfilePackageMutation,
} from '@deepseek-ai/dsh-app-boot'
import { parseDesktopPluginSource } from './plugin-source.ts'

// The Host may have submitted a verified source with a private signed URL.
// Stage errors can contain pnpm arguments, paths, or that URL: never echo them
// through the Host's public Remote/log surface or the renderer.
...
export class DesktopPackageTransactionIpc {
      ...
      switch (input.operation) {
        ...
        case 'stage': {
          ...
          try {
            ...
            // A requested abort is not proof of cleanup: only the backend's
            // explicit post-cleanup acknowledgement may become `cancelled`.
            acknowledgedCancellation = abort.signal.aborted && error instanceof ProfilePackageCancelledError
            throw error
  ...
  async dispose(): Promise<void> {
    ...
    const failures = outcomes.flatMap((outcome, index): unknown[] => {
      ...
      return reason instanceof ProfilePackageCancelledError && stages[index]?.abort.signal.aborted === true ? [] : [reason]
    })
    // Failure is retained as a failed stop, but not with raw source/argv/text in
    // an AggregateError that a Host or renderer could log across the boundary.
    if (failures.length > 0) throw new Error('desktop packages: owned staging cleanup failed during Host disposal')
  }
}
