...
The next Desktop process stages the packaged plugin plan with its runtime, starts the final-location Host, and accepts completion only when helper results, installed files, capability, plan, actual plugin inventory, receipts, and sequence agree. Missing, interrupted, blocked, conflicting, or mismatched completion evidence opens recovery without recording success. From a verified `dsh-windows-ops` checkout, recovery for the legacy migration uses `pwsh -NoProfile -File .\Install-DshOfficialDesktop.ps1 -Action Complete`.
