...
The current maintenance plan keeps Core `0.1.6-alpha.1` and pins Copilot `0.4.0-alpha.30`. Its read-only acceptance covers the alpha.29 provider-only settings and alpha.30 signed-out Manage state. Copilot's own synthetic Client tests cover Desktop same-window verification handoff, Web new-tab behavior, and the selectable manual verification address; Desktop packaging does not initiate that flow. Alpha.28 request-budget and compaction behavior and alpha.29 account-owned search-model resolution remain hash-bound plugin behavior that requires separate model/search acceptance. The [managed Copilot maintenance decision](../../.agents/notes/implemented/architecture/2026-09-20-managed-desktop-copilot-maintenance.md) records exact publication evidence, official Core alpha.2 overlap, retained gaps, and migration conditions.
