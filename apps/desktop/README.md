...
The current maintenance plan keeps Core `0.1.6-alpha.1` and pins Copilot `0.4.0-alpha.32`. Its read-only acceptance preserves the alpha.29 provider-only settings and alpha.30 signed-out Manage state, then binds alpha.32's required account-usage capability and verifies that fresh signed-out initial and restart pages emit no quota control or credit summary. The packaged smoke has no Host-request instrumentation; the immutable plugin's gateway regression owns the no-startup/signed-out-network evidence, and neither source claims live quota access. Alpha.31 HTTP 401 recovery also remains source-and-plugin-CI evidence because exercising it requires a live model rejection. The [managed Copilot maintenance decision](../../.agents/notes/implemented/architecture/2026-09-20-managed-desktop-copilot-maintenance.md) records exact publication evidence, official Core alpha.2 overlap, retained gaps, and migration conditions.
