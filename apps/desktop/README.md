...
| Updates | Independent shell and dsh updates would recreate version splits, while unsigned enterprise builds cannot weaken native publisher verification. | Signed releases use the native updater. A Windows Ops build can instead carry one source-locked managed-update capability and detached helper; the two modes are mutually exclusive and both replace the complete Desktop release. |
...
### Windows Ops managed updates

Signed packages that contain `resources/app-update.yml` use `electron-updater` and retain its publisher and platform-signature checks. A Windows Ops package selects the separate managed mode only by carrying both `resources/managed-update/capability.json` and the packaged `resources/managed-update/helper.mjs`; startup rejects a package that enables both modes. The capability locks one immutable HTTPS `cloga/deepseek-harness` release manifest by versioned GitHub Release URL, canonical manifest SHA-256, minimum sequence, expected Desktop version, and expected source commit. An optional `cloga/dsh-windows-ops` migration manifest is accepted only at installed sequence zero.

**Check for updates** fetches only the capability URL, follows only credential-free HTTPS redirects through GitHub release asset hosts, and validates the exact schema, canonical self-hash, sequence, source repository, tag, commit, installer hashes, installed evidence, and verified GitHub Release plugin source. Renderer messages cannot supply a URL, executable, process id, path, or installer argument.

**Install** reports running Sessions, queued messages, active jobs, the current composer draft, attachments, and submission state before one confirmation. After confirmation, Electron writes a one-time handoff under its user-data directory and starts a copied Node.js plus standalone helper. Electron keeps the application and Host running unless the helper validates the selected manifest and acknowledges the same manifest hash. After acknowledgement, Electron stops only its owned Host and exits normally. The helper waits only for the recorded Electron and Host process ids, downloads and revalidates the build receipt and installer, stages them under the operation directory, checks the declared unsigned Authenticode state, removes credential-shaped environment variables from child processes, and starts the NSIS installer with no silent arguments. Windows warnings and UAC remain interactive user decisions.

The next Desktop process accepts completion only when the helper result, pending marker, manifest, installed executable, runtime descriptor, managed sequence, and plugin-provision receipt all match. It installs the release-locked plugin through the verified GitHub Release transaction before starting the active Host. Missing, interrupted, blocked, conflicting, or mismatched evidence opens startup recovery instead of reporting success. From a verified `dsh-windows-ops` checkout, recovery uses `pwsh -NoProfile -File .\Install-DshOfficialDesktop.ps1 -Action Complete`.

Windows Ops owns release publication and injects the capability for its unsigned managed build; the renderer and ordinary Desktop packaging do not create or edit it. The source release manifest remains the release identity, and Windows Ops consumes that record rather than maintaining a second release definition. Once managed Windows builds use signed native artifacts with publisher validation, omitting the capability removes the managed path without changing the native updater.
