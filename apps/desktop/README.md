...
A managed fork release may carry `resources/desktop-provisioning/plan.json`. The exact-state plan lists external plugins without adding them to `desktop-runtime.json.sharedPackages`; Desktop keeps `@deepseek-ai/cordis` and `@deepseek-ai/dsh-*` packages application-owned, installs each external root from its verified release tgz, and enables its bundle in the reserved profile. The normal Host composition then loads the plugin's server patch, while `dsh.client` and `./client` make its Client contribution available to Settings. The plan is generic; GitHub Copilot appears in Settings > Models only when the reviewed release plan names the independently released `dsh-github-copilot` package.

Each entry is `required` or optional and contains a `githubRelease` source with a checksum-manifest lock. The checksum asset is JSON with exactly `schemaVersion`, `packageName`, `version`, `asset`, `sha256`, and `integrity`; those package fields must equal the source lock. Both the tgz and checksum asset are pinned by GitHub Release asset identity, byte size, SHA-256, and SHA-512 SRI, and the Release tag is pinned to an exact commit. Every entry in one plan uses the same credential-free HTTPS dependency registry.

Windows Ops changes `desktopProvisioning` in [`release/cloga-windows-x64.json`](release/cloga-windows-x64.json), then runs the protected `desktop-fork-release.yml` workflow. The workflow's prepare step writes the reviewed plan, sets `DSH_DESKTOP_PLUGIN_PROVISIONING_PLAN` for packaging, and embeds the plan plus capability schema 3. Finalization publishes `desktop-provisioning.json`, includes its file and canonical plan hashes in `build-receipt.json`, and covers it with `SHA256SUMS` and `SHA512SUMS`. A release containing a non-empty plan is required before Windows Ops can deploy the plugin; this source change does not publish one.
