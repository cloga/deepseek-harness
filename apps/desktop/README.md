...
Each entry is `required` or optional and contains a `githubRelease` source with a checksum-manifest lock. The artifact lock names the exact Release asset id, filename, byte size, and SHA-256. The checksum lock names the exact asset id, canonical GitHub Release URL, filename, byte size, SHA-256, and `sha256sums` format. Desktop verifies the checksum asset and requires exactly one `<sha256>  <artifact>` entry for the package; missing, duplicate, malformed, renamed, or mismatched entries fail the transaction. Artifact and checksum SHA-512 SRI fields are optional, but Desktop verifies each one when the release plan supplies it. This accepts the immutable `SHA256SUMS` assets already published by Windows Ops plugins without weakening Release, tag, commit, redirect, identity, size, or SHA-256 checks. Every entry in one plan uses the same credential-free HTTPS dependency registry.

Windows Ops changes `desktopProvisioning` in [`release/cloga-windows-x64.json`](release/cloga-windows-x64.json), then runs the protected `desktop-fork-release.yml` workflow. Existing immutable plugin releases remain valid inputs: use their versioned tgz and `SHA256SUMS` assets rather than republishing them. The workflow's prepare step writes the reviewed plan, sets `DSH_DESKTOP_PLUGIN_PROVISIONING_PLAN` for packaging, and embeds the plan plus capability schema 3. Finalization publishes `desktop-provisioning.json`, includes its file and canonical plan hashes in `build-receipt.json`, and covers it with `SHA256SUMS` and `SHA512SUMS`. A release containing a non-empty plan is required before Windows Ops can deploy the plugin; this source change does not publish one.

At startup Desktop reconciles only receipt-owned plugins to the packaged plan. It preserves manually installed plugins, removes receipt-owned packages omitted by the exact plan, stages replacements, checks the complete Host and Client composition, and atomically activates the result. A required plugin failure preserves the active profile. If an optional plugin makes the staged composition unhealthy, Desktop activates the remaining composition with that package disabled and records the failure. Restart and profile regeneration reuse the durable state only while the plan hash, package version, receipt-backed source, artifact, and enabled state still match.

Windows Ops verifies `resources/managed-update/capability.json` for `desktopNativePluginProvisioning`, the packaged and published plan hashes, `desktop-plugin-receipts.json` for release and artifact identity, and `$DSH_HOME/profiles/desktop/desktop-plugin-provisioning-state.json` for each plugin's `active` or `optional-failed` composition status plus removed-package evidence. Managed-update completion also compares the installed plan with the build-carried capability before recording the sequence. Removal is an empty or reduced exact plan; activation failure rolls back the complete previous profile.
...
The next Desktop process reconciles the packaged plugin plan before Host startup and accepts completion only when the helper result, pending marker, manifest, installed executable, runtime descriptor, packaged plan, build capability, and managed sequence all match. An acknowledged handoff without terminal state, or any other missing, interrupted, blocked, conflicting, or mismatched evidence, opens startup recovery instead of starting a new Host or reporting success. From a verified `dsh-windows-ops` checkout, recovery for the legacy migration uses `pwsh -NoProfile -File .\Install-DshOfficialDesktop.ps1 -Action Complete`.
