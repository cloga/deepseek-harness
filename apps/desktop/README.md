...
1. The main window displays a local loading page before profile preparation or backend startup. A fresh profile creates its manifest and shared package links while preserving unrelated files. Unchanged runtime links are reused; a packaged plugin plan also requires an exact observation of installed versions, receipts, local artifact hashes, and enabled state.
2. Application upgrades stage target runtime links and the desired plugin inventory together before checking peers. Obsolete release-owned plugins cannot block their planned replacement or removal. Profile configuration and manual plugin versions are retained.
3. Every changed profile copies metadata and retained artifacts, never `node_modules`. Bundled pnpm reconstructs private dependencies in staging with scripts disabled, validates and links host packages, then runs approved pending builds and validates again. Runtime upgrades never run package operations against the active reserved profile.
...
The plugin window accepts the following source inputs. Installation requires a package with a real name, an exact package version, a `dsh.bundle.patch`, and its declared prebuilt Host and Client files. A repository name does not determine the installed package name.
...
Use **Reinstall from source** to review or edit the source in the in-page dialog; use an absolute path when selecting a local file again. Explicit reinstallation can select different bytes or a different commit even when the package version is unchanged. Registry updates continue to select versions. Verified Release updates use the verified channel and do not silently fall back to a registry or general source. Replacing a package's source clears the opposite source-lock or receipt ownership rather than presenting both as current provenance.
...
Windows Ops changes `desktopProvisioning` in [`release/cloga-windows-x64.json`](release/cloga-windows-x64.json), then runs the protected `desktop-fork-release.yml` workflow. Use existing immutable versioned tgz and `SHA256SUMS` assets rather than republishing them. Prepare sets `DSH_DESKTOP_PLUGIN_PROVISIONING_PLAN` for packaging and embeds the plan plus capability schema 3. Finalization rejects disagreement between reviewed inputs, packaged capability and plan, published bytes, and receipt hashes. It publishes the packaged plan as `desktop-provisioning.json`, records both its file hash and canonical plan hash in `build-receipt.json`, and covers release files with `SHA256SUMS` and `SHA512SUMS`. Deployment requires a release with the actual non-empty provider plan.

At startup Desktop reconciles release-owned plugins to the packaged plan while preserving manual registry, source-snapshot, and verified-release plugins outside the plan's package names, including their enabled states. A desired name follows the plan's exact source even when a different package was manually installed under that name. Required entries form a validated baseline. Each optional entry is added to a separate candidate; download, validation, install, graph, or health failure excludes only that entry and records its phase and reason without a successful receipt. Required failures preserve the active profile. Reuse requires exact desired/result membership, matching sources, receipts, versions, artifact bytes, and enabled states, with no extra release-owned roots. An empty plan removes only release-owned roots.

The private receipt store records user or release ownership separately from source verification. Explicit manual verified installation records user ownership, including a reinstall of the same source; rebuilding that exact source retains the user ownership. Legacy ownership is inferred only from an active, identical receipt in consistent prior provisioning state and its matching manifest reference; other verified plugins remain user-owned. Legacy records cannot distinguish a manual reinstall that left exactly the same receipt. Ownership migration and changes commit or roll back with the staged profile. The [plugin retention decision](../../.agents/notes/implemented/bug-fix/2026-09-17-desktop-plugin-retention-and-lockfiles.md) owns these deletion and migration rules.

Before frozen reconstruction, repairs for verified-release roots require a Windows separator mismatch whose receipt, manifest, locked package identity, tarball path, and artifact SHA-256/SHA-512 agree. The repair changes the lockfile specifier in staging and rejects symbolic-link lockfiles; unrelated dependency drift still fails frozen validation.
...
Package transactions hold `$DSH_HOME/desktop/profile.lock` through pnpm process exit and activation. The two directory renames are journaled in `$DSH_HOME/desktop/profile-activation.json`; startup restores an interrupted uncommitted profile under the same lock. If restoration fails, retain the journal and its named `.desktop-transaction-*` directory for recovery, including `rollback`; do not delete them or run pnpm in the reserved profile. Shared links use symlinks or Windows junctions, and cleanup never follows them. Native builds remain constrained by the profile's reviewed `allowBuilds` list.

### Fork-owned Windows managed updates

Desktop checks for updates automatically ten seconds after startup. You can also use **Check for updates** in the application menu. An available update asks for confirmation before Desktop downloads, verifies, and opens the installer; choosing **Later** does not install it. You do not need to download an installer manually. This is an interactive update, not an unattended installation: Windows warnings, installer choices, and UAC remain yours to approve.
...
The next Desktop process stages the packaged plugin plan with its runtime, starts the final-location Host, and accepts completion only when helper results, installed files, capability, plan, actual plugin inventory, receipts, and sequence agree. Completion uses the sequence in the durable completion receipt, not the packaged sequence used to prevent self-selection during discovery. A newly packaged version does not prove its pending installation completed. Missing, interrupted, blocked, conflicting, or mismatched completion evidence opens recovery without recording success. From a verified `dsh-windows-ops` checkout, recovery for the legacy migration uses `pwsh -NoProfile -File .\Install-DshOfficialDesktop.ps1 -Action Complete`.
