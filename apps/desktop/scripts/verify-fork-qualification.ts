...
export function verifyForkQualification(options: ForkQualificationOptions) {
  ...
  const helper = input(options.packagedEvidence, 'helper-acceptance.json', 'packaged.helper').value
...
export function runForkQualificationCli(): void {
  const names = ['plan', 'release-assets', 'packaged-evidence', 'upgrade-root', 'baseline-directory', 'expected-source', 'run-id', 'run-attempt', 'output']
  ...
  const options = { planPath: join(repository, 'apps/desktop/release/cloga-windows-x64.json'), releaseAssets,
    packagedEvidence: join(repository, 'dist/desktop-copilot-acceptance'), upgradeRoot: join(temporary, `cloga-installer-upgrade-${runId}-${runAttempt}`),
    baselineDirectory: join(temporary, `desktop-upgrade-baseline-${runId}-${runAttempt}`), expectedSource: source, runId, runAttempt }
  for (const [argument, canonical] of [['plan', options.planPath], ['release-assets', releaseAssets], ['packaged-evidence', options.packagedEvidence],
