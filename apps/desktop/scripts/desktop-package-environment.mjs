...
import { resolveDesktopAppId, resolveDesktopForkReleaseEnvironment, resolveMacOSNotarizationEnvironment, resolveMacOSSigningEnvironment } from './desktop-release-environment.mjs'
 ...
 * @param {{ unsigned?: boolean, arch?: string }} options Explicit mode; only a validated unsigned Windows x64 fork uses capability-owned settings.
 * @returns {NodeJS.ProcessEnv} Isolated environment with file-owned official settings or the explicit fork capability inputs.
 */
export function loadDesktopPackageEnvironment(platform, environment = process.env, appRoot = APP_ROOT, options = {}) {
  const fork = resolveDesktopForkReleaseEnvironment(environment)
  if (fork !== undefined) {
    if (platform !== 'win32' || options.arch !== 'x64' || options.unsigned !== true) {
      throw new Error('desktop package: managed fork settings require explicit unsigned Windows x64 packaging')
    }
    return {
      ...Object.fromEntries(Object.entries(environment).filter(([name]) => !AMBIENT_RELEASE_SETTING.test(name)
        && name !== 'DSH_DESKTOP_RELEASE_GITHUB_TOKEN')),
      DSH_DESKTOP_APP_ID: fork.appId,
...
export function validateDesktopPackageEnvironment(environment, target, options = {}) {
  ...
  const fork = resolveDesktopForkReleaseEnvironment(environment)
  if (fork !== undefined) {
    if (!options.unsigned || target.platform !== 'win32' || target.arch !== 'x64' || options.prepareOnly) {
      throw new Error('desktop package: managed fork settings require explicit unsigned Windows x64 packaging')
    }
    return
  }
