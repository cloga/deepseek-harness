...
每个条目分为 `required` 或 optional，并包含带 checksum-manifest lock 的 `githubRelease` source。Artifact lock 指定精确的 Release asset id、文件名、字节大小与 SHA-256。Checksum lock 指定精确的 asset id、规范 GitHub Release URL、文件名、字节大小、SHA-256 与 `sha256sums` 格式。Desktop 会验证 checksum asset，并要求 package 存在且只存在一个 `<sha256>  <artifact>` 条目；缺失、重复、格式错误、重命名或 hash 不匹配都会使事务失败。Artifact 与 checksum 的 SHA-512 SRI 字段可选，但 release plan 一旦提供，Desktop 就必须验证。这样可以直接使用 Windows Ops 插件已发布且不可变的 `SHA256SUMS` 资产，同时不削弱 Release、tag、commit、redirect、identity、size 或 SHA-256 检查。同一个 plan 的每个条目使用相同的无凭据 HTTPS dependency registry。

Windows Ops 修改 [`release/cloga-windows-x64.json`](release/cloga-windows-x64.json) 中的 `desktopProvisioning`，然后运行受保护的 `desktop-fork-release.yml` workflow。现有不可变插件 release 仍然是有效输入：直接使用其中带版本的 tgz 与 `SHA256SUMS` 资产，不需要重新发布。Workflow 的 prepare 步骤写出经过评审的 plan，为 packaging 设置 `DSH_DESKTOP_PLUGIN_PROVISIONING_PLAN`，并嵌入 plan 与 capability schema 3。Finalization 发布 `desktop-provisioning.json`，在 `build-receipt.json` 中记录其文件 hash 与规范 plan hash，并通过 `SHA256SUMS` 和 `SHA512SUMS` 覆盖它。Windows Ops 必须等包含非空 plan 的新 release 发布后才能部署插件；本次源码变更本身不发布 release。
