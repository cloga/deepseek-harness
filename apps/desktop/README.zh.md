...
官方 alpha2 导航已将外部 HTTP 和 HTTPS 目标交给系统浏览器，并拒绝创建 Electron 弹出窗口。同窗口 `dsh-app:` 导航以及到当前自有 Host HTTP origin 的导航保留在内部；其他外部 URI scheme 仍被阻止。Fork 额外拒绝格式错误的目标，并接住浏览器打开操作的同步失败与异步拒绝，以不暴露 URL 或底层错误的本地化建议报告。失败报告不能替换应用文档，也不改变原生致命恢复、顶栏／上下文菜单或共享 PluginManager。Web 客户端浏览器预览保持不变；[外链决策](../../.agents/notes/implemented/bug-fix/2026-09-20-desktop-external-links.zh.md)说明安全和验证边界。
...
alpha2 候选计划固定不可变 Copilot `0.4.0-alpha.35`，保留 Session selector 修复、alpha.34 的 Model roles 退役及 alpha.35 的 Credits 呈现。现有官方 alpha2 公开 dock 与 ContextMeter 位置仍负责布局；换行及空 outlet 处理不改变模型路由、token 统计或计费。精确来源发布计划为权威；目标版本本身不构成发布或验收证据。验收保留仅提供方设置、登出 Manage、初始／重启缺席检查和必需的 `account-quota-composer-usage` capability。[隔离验收检查](#isolated-provisioning-acceptance)将合成正向 quota 与使用未登录 Host 的原生持久化 Session 几何检查分开。这些 DOM 观察没有 Host request instrumentation，不证明实时 quota access 或 Session 计费；插件 gateway regression 拥有无启动／登出网络请求证据。`auth-rejection-recovery` capability 的 HTTP 401 处理仍属于源码与插件 CI 证据，而非实时模型拒绝验收。
