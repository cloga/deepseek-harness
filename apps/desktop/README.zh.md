...
当前已发布的不可变基线是 `0.1.6-alpha.1.cloga.6`，sequence 为 16。alpha2 候选版本是 `0.1.6-alpha.2.cloga.1`，暂定 sequence 为 17；候选版本不会预留 sequence，发布前必须重新检查通道。其 plan 不构成发布或已安装升级证据。安装器升级 fixture（测试前置数据）仍锁定 `0.1.6-alpha.1.cloga.2`，sequence 为 12，不证明从 `.cloga.6` 升级已通过验收。
