...
Desktop 在启动时把 release-owned 插件协调到打包 plan，同时保留手动 registry、来源快照与 verified-release 声明，包括已禁用插件。Schema 1 和 schema 2 的 `strict-pin` 条目要求精确计划来源。Schema 2 的 `compatible-user-override` 条目可以改用已启用、user-owned、具有校验和证明、GitHub owner、repository、包名及规范化 dependency registry 都相同且保留制品完整的来源；版本顺序不是兼容证据。只有完整暂存依赖图和目标运行时 Host 通过健康检查，Desktop 才接受该选择；状态分别记录请求来源与有效来源，并在运行时变化后重新检查。如果恰好一个覆盖处于活动状态时暂存健康检查失败，Desktop 会保留活动 profile，并把该包的计划来源作为恢复选择，但不会声称覆盖导致了失败；存在多个覆盖时不会任意提供限定目标的选择。禁用全部插件和重置仍是更广泛的后备操作。Required 条目构成经过验证的基线。每个 optional 计划来源 acquisition 在独立 candidate 中测试；失败会记录阶段与原因，不保留成功 receipt。复用要求 plan/result 成员、请求与有效来源、策略、receipt、产物字节、归属及启用状态完全匹配，且没有额外 release-owned 根包。空 plan 只删除 release-owned 根包。
