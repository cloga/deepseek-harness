...
test('scopes weighted approval to the canonical upstream repository', () => {
  assert.equal(isCanonicalApprovalRepository(pullRequestEvent()), true)
  assert.equal(isCanonicalApprovalRepository({
    ...pullRequestEvent(),
    repository: { full_name: 'cloga/deepseek-harness' },
  }), false)
})
...
test('resolves a review workflow run to the current pull request and rejects stale heads', async () => {
  ...
  const current = await approvalEventFromWorkflowRun({
    ...
    api: async path => {
      assert.equal(path, '/repos/deepseek-ai/deepseek-harness/pulls/42')
...
test('publishes the required status and replaces stale success with error on evaluation failure', async () => {
  ...
  assert.deepEqual(calls.at(-1), {
    path: `/repos/deepseek-ai/deepseek-harness/statuses/${HEAD_SHA}`,
    options: {
      method: 'POST',
      body: {
        state: 'success',
...
test('the publisher counts merged history through the production API path', async () => {
  ...
  const result = await runWithHistory({
    ...
    api: async (path, options) => {
      if (path.includes('/comments?')) return []
      if (path === '/graphql') {
        assert.equal(options.body.variables.owner, 'deepseek-ai')
