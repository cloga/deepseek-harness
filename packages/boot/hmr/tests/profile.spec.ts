...
it('rejects a profile launcher that omits application readiness', async () => {
  ...
  ctx.provide('profileContext', { watchProfilePatches: true } as ProfileContext)
  await expect(ctx.plugin(Hmr, { root: [], ignored: [], debounce: 0 })).rejects.toThrow('Profile HMR requires application readiness')
})

it.each(['startup-frozen', 'application-owned'] as const)(
  'never adds implicit profile or manifest watchers for %s profiles', async (_mode) => {
    const ctx = new Context()
    ctx.baseUrl = pathToFileURL(process.cwd() + '/').href
    onTestFinished(() => ctx.fiber.dispose())
    await ctx.plugin(Loader)
    await ctx.plugin(Timer)
    ctx.provide('profileContext', { watchProfilePatches: false } as ProfileContext)
    const start = watchers.length
    await ctx.plugin(Hmr, { root: [], ignored: [], debounce: 0 })
    expect(ctx.get('hmr')).toBeDefined()
    expect(watchers.slice(start)).toHaveLength(1) // only the opt-in module watcher; no profile/home/manifest watches
  },
)
