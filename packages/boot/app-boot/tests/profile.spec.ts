...
describe('healIsolatedProfileModuleFallback', () => {
  it.each([false, true])('keeps installation peers profile-local without replacing pnpm packages (Web fallback: %s)', async (webFallback) => {
    const home = tmp()
    ...
    for (const consumer of [consumerA, consumerB]) {
      ...
      writeFileSync(join(consumer, '..', 'package.json'), JSON.stringify({ name: 'custom-plugin', peerDependencies: { commander: '*' } }))
    }
...
describe('unlinkProfileModuleFallback', () => {
  it('detaches only owned fallback links, not packages or another profile', () => {
    const home = tmp()
