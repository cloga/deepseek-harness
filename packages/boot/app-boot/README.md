...
- **Profile launch data.** `ctx.profileContext` contains profile locations, startup bundle names, parsed invocation overlays, the telemetry opt-out value, optional bundled package-manager facts, and the launcher's staging requirement. `readProfilePatches()` composes the supplied startup profile or reads current files at those locations; callers schedule and apply the result.
- **Launcher-owned package staging.** The versioned `profilePackageTransactions` contract separates a prepared graph from activation. Import its serializable source and prepared-result types from `@deepseek-ai/dsh-app-boot/types` without loading Host boot code. Ordinary staging retains its five-field prepared result; only pending/status/list queries additionally accept the versioned bundle-selection result through `ProfilePendingPackageChange`. Neither result is an active receipt or activation authority. The shared sibling lease survives directory replacement; callers acquire it before HMR and never reacquire it through an IPC callback. The launcher supplies the backend before profile rows mount, and required-but-unavailable staging rejects startup. [Plugin Manager](../plugin-manager/README.md#launcher-owned-staging) owns the user entry point.
