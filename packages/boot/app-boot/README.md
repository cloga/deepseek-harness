...
- **Launcher-owned package staging.** The versioned `profilePackageTransactions` contract separates a prepared graph from activation. Its UUID and scalar fingerprint result is not an active receipt. The shared sibling lease survives directory replacement; callers acquire it before HMR and never reacquire it through an IPC callback. The launcher supplies the backend before profile rows mount, and required-but-unavailable staging rejects startup. [Plugin Manager](../plugin-manager/README.md#launcher-owned-staging) owns the user entry point.
