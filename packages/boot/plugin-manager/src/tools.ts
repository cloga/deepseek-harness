...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    name: 'plugin_manager',
    description: 'List plugins or bundles in the current profile, enable or disable them, install a bundle, or remove an installed bundle. Every action requires danger-full-access permission or approval for this call. Approval does not change the session permission mode. Changes affect every session in this profile. List first to obtain exact identifiers. Package installation can execute allowed build scripts. Ordinary live profiles apply changes immediately; startup profiles require restart. Launcher-staged profiles return prepared changes with the active profile untouched; activation requires separate shell confirmation. Use list_pending to recover transaction identities and cancel_pending to discard a stage without removing an active plugin.',
    parameters: {
      action: { type: 'string', required: true, enum: ['list_plugins', 'list_bundles', 'list_pending', 'cancel_pending', 'set_plugin', 'set_bundle', 'install_bundle', 'remove_bundle'], description: 'Management operation.' },
      target: { type: 'string', description: 'Plugin entry id, bundle package name, installation spec, or prepared transaction UUID, according to action.' },
      enabled: { type: 'boolean', description: 'Required for set operations; defaults to true for installation.' },
      approvedBuilds: { type: 'array', items: { type: 'string' }, description: 'For install_bundle: pass names from pendingBuilds only after the user explicitly approves running their install scripts in the conversation. This grants persistent permission in the resulting profile; on staged launchers it does not change the active profile before activation.' },
    ...
    async execute(args, exec) {
      ...
      switch (args.action) {
        ...
        case 'list_pending': {
          ...
          const rows = args.action === 'list_plugins' ? await manager.listPlugins()
            : args.action === 'list_bundles' ? await manager.listBundles() : await manager.listPendingPackageChanges()
        ...
        case 'cancel_pending':
          if (args.target === undefined) throw new Error('target transaction id is required')
          await manager.cancelPendingPackageChange(args.target)
          return JSON.stringify({ cancelled: args.target, activeProfileChanged: false })
        case 'set_plugin':
        case 'set_bundle': {
