...
export class PluginManager extends TypertRemoteService {
   ...
   */
  @Remote
  installBundle(spec: string | ProfileVerifiedReleaseSource, options?: InstallBundleOptions): Promise<ChangeResult> {
    if (this.profile.stagedPackageTransactions === true) return this.installStagedBundle(spec, options)
    if (typeof spec !== 'string') return Promise.resolve({
      stage: 'install', target: spec.packageName, changed: false, application: 'failed',
  ...
  removeBundle(name: string): Promise<ChangeResult> {
    if (this.profile.stagedPackageTransactions === true) return this.stagePackage(randomUUID(), { kind: 'remove', name },
      { stage: 'remove', target: name }, undefined, async () => {
  ...
  private installStagedBundle(spec: string | ProfileVerifiedReleaseSource, options?: InstallBundleOptions): Promise<ChangeResult> {
    ...
    const result = this.stagePackage(requestId, {
      kind: 'install',
      source: typeof spec === 'string' ? { schemaVersion: 1, type: 'packageSpec', spec } : spec,
  ...
  /** Delegate before the stock file snapshot, build approval, pnpm, enablement, or HMR path. */
  private stagePackage(
    ...
    const task = (async (): Promise<ChangeResult> => {
      ...
      try {
        ...
        else result.error = managementError(error)
      }
      return result
    })()
  ...
  private async change(
    ...
    if (this.profile.stagedPackageTransactions === true && this.ownerContext.get('profilePackageTransactions')?.protocolVersion !== 1) {
      throw new Error('Launcher package staging is required but unavailable')
    }
    const lease = this.profile.stagedPackageTransactions === true
      ? profilePackageLeaseTarget(this.profile.dir) : join(this.profile.dir, 'package.json')
    return withFileLock(lease, async () => {
      ...
      return result
    }, { waitMs: this.profile.stagedPackageTransactions === true ? 0 : this.lockWaitMs })
  }
