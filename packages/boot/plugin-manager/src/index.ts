...
/** One installation result, including a writer-lock rejection before a result exists. */
...
export class PluginManager extends TypertRemoteService {
  ...
  /** Desktop graph changes belong to its launcher, never to this stock live-profile writer. */
  private refuseUnownedStage(stage: ChangeResult['stage'], target: string, enabled?: boolean): Promise<ChangeResult> {
    return Promise.resolve({ stage, target, ...(enabled === undefined ? {} : { enabled }), changed: false,
      application: 'failed', error: { code: 'operation-error', diagnostic: 'Desktop package mutations require launcher-owned staging' } })
  }
  ...
  listBundles(): Promise<BundleInfo[]> {
    ...
    for (const name of names) {
      ...
      // The maintained installation has no declared optional-offer list. Do not
      // mislabel release-planned Copilot or arbitrary installation dependencies.
  ...
  setPluginEnabled(id: PluginEntryId, enabled: boolean): Promise<ChangeResult> {
    if (this.profile.stagedPackageTransactions === true) return this.refuseUnownedStage('enable', id, enabled)
  ...
  setBundleEnabled(name: string, enabled: boolean): Promise<ChangeResult> {
    if (this.profile.stagedPackageTransactions === true) return this.refuseUnownedStage('enable', name, enabled)
    return this.change(result => this.configure(async () => {
  ...
  installBundle(spec: string, options?: InstallBundleOptions): Promise<ChangeResult> {
    ...
    /* v8 ignore next -- change() folds ordinary failures; a lock or disposal failure rejects before an outcome exists. */
  ...
  async cancelInstall(requestId: PluginInstallRequestId): Promise<PluginInstallCancellation> {
    ...
    // An already durable or applied result is not a cancelled operation.
