...
async function fixture(mode: 'read-only' | 'workspace-write' | 'danger-full-access' = 'danger-full-access', approval?: 'ask' | 'never') {
  ...
  const manager = {
    ...
    listPendingPackageChanges: vi.fn(async () => [{ transactionId: '11111111-1111-4111-8111-111111111111', state: 'prepared' }]),
    cancelPendingPackageChange: vi.fn(async () => {}),
...
it.each(['read-only', 'workspace-write'] as const)('denies every management action in %s before accessing the manager', async (mode) => {
  ...
  for (const action of ['list_plugins', 'list_bundles', 'list_pending', 'cancel_pending', 'set_plugin', 'set_bundle', 'install_bundle', 'remove_bundle']) {
...
it('recovers and discards stages through the same manager without removing an active bundle', async () => {
  const { call, manager } = await fixture()
  const id = '11111111-1111-4111-8111-111111111111'
  expect(resultText(await call({ action: 'list_pending' }))).toContain(id)
  expect(JSON.parse(resultText(await call({ action: 'cancel_pending', target: id })))).toEqual({ cancelled: id, activeProfileChanged: false })
  expect(manager.cancelPendingPackageChange).toHaveBeenCalledExactlyOnceWith(id)
...
it.each([
  ...
  { action: 'install_bundle' }, { action: 'remove_bundle' }, { action: 'cancel_pending' },
  { action: 'list_pending', limit: 101 },
