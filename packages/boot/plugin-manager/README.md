...
<a id="launcher-owned-staging"></a>
### Launcher-owned staging

A launcher can require package staging through `profileContext.stagedPackageTransactions` and the versioned `profilePackageTransactions` service. The manager remains the sole Web and tool entry point, but install and remove delegate before any in-place pnpm run, build approval, bundle selection, or unload. An absent or incompatible backend refuses the operation rather than falling back. A structured verified Release source is accepted only on this path.

A successful stage returns `application: 'prepared'`, `changed: false`, and a durable transaction UUID; it is not an installed package or an active receipt. Pending queries can read either the legacy single-package result or a versioned bundle-selection result naming every actual target; ordinary staging and `ChangeResult.prepared` retain the legacy result. Pending records are readable after reconnect and can be discarded through the same manager, but their visibility does not authorize activation. The launcher separately owns activation, fresh admission checks, rollback, and post-readiness receipts. Live configuration edits use the launcher's stable sibling lease before HMR. The ordinary in-place installation and rollback rules elsewhere on this page do not describe delegated stages.
