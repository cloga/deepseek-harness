...
A successful stage returns `application: 'prepared'`, `changed: false`, and a durable transaction UUID; it is not an installed package or an active receipt. Pending stages are recoverable after reconnect and can be discarded through the same manager. The launcher separately owns activation, fresh admission checks, rollback, and post-readiness receipts. Live configuration edits use the launcher's stable sibling lease before HMR. The ordinary in-place installation and rollback rules elsewhere on this page do not describe delegated stages.
