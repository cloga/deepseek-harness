...
启动器可以通过 `profileContext.stagedPackageTransactions` 和带版本号的 `profilePackageTransactions` 服务要求包操作先暂存。管理器仍是 Web 与工具的唯一入口，但安装和删除会在原地执行 pnpm、批准构建脚本、选择组合包或卸载之前委托给后端。后端缺失或版本不兼容时拒绝操作，不回退到原地修改。结构化的已验证 Release 来源只在此路径上接受。

暂存成功返回 `application: 'prepared'`、`changed: false` 及持久事务 UUID；这不表示包已安装，也不是已激活的收据。待处理查询可读取旧有单包结果，或列出全部实际目标的带版本组合包选择结果；普通暂存及 `ChangeResult.prepared` 仍使用旧结果。重新连接后可读取待处理记录，并通过同一管理器丢弃，但能够看到记录不代表获得激活授权。启动器单独负责激活、重新检查请求接纳条件、回滚及就绪后的收据。实时配置编辑先获取启动器位于 profile 外的稳定兄弟锁，再进入 HMR。本页其他位置描述的普通原地安装与回滚规则不适用于委托暂存。
