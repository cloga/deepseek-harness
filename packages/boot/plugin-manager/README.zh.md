...
暂存成功返回 `application: 'prepared'`、`changed: false` 及持久事务 UUID；这不表示包已安装，也不是已激活的收据。重新连接后可恢复待处理的暂存记录，并通过同一管理器丢弃。启动器单独负责激活、重新检查请求接纳条件、回滚及就绪后的收据。实时配置编辑先获取启动器位于 profile 外的稳定兄弟锁，再进入 HMR。本页其他位置描述的普通原地安装与回滚规则不适用于委托暂存。
