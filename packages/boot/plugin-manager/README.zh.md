...
> 本 fork 当前仅导入源码；base／Web 尚未挂载管理器。Desktop 包变更仍归启动器所有，Electron 事务所有者接通前，暂存模式的变更方法都会在写盘前明确拒绝。

应用拥有的 profile 完成安全接线后，启动器信息才可提供内置包管理器调用方式。它在包操作和 registry 检查中优先于 `pnpmCommand`；其环境仅应用于这些子进程。
...
本 fork 的 base／Web 尚未挂载管理服务，也没有随附插件管理页或模型工具。将来审查 Web 所有者并接线后，侧边栏才可管理组合包及可唯一定位的行；设置页插件清单仍保持只读。Agent 预设条目也只读。只有 profile 与权限所有者接通后，显式启用的 `plugin_manager` 工具才能提供相同操作。 每个工具操作都要求 `danger-full-access` 或本次调用的批准。在较低沙箱模式下，`ask` 会请求审批；`never`、拒绝、取消或审批渠道不可用时均不执行。批准不改变会话的权限模式。profile 变更跨会话持久化，已安装的 Host 代码在宿主进程内运行，不受工作区沙箱限制。依赖构建脚本仍需单独批准。
...
`installBundle` 接受调用方生成的 `requestId`，`plugin-manager/install-log` 在其下流式转发每次 pnpm 运行的输出，`plugin-manager/install-state` 通告 `installing`、`cancelling` 与 `applying`。复用尚在运行的请求 id 会在再次运行 pnpm 前失败，也不会取代原请求的取消所有者。`cancelInstall(requestId)` 停止运行，只在 pnpm 退出且文件恢复后答复 `cancelled`，组合包已在应用时答复 `too-late`，其他 id 答复 `not-running`；安装调用随后报告 `application: 'cancelled'`。失败、被取消或装入了没有组合包 patch 的包的运行，会把 `package.json` 与 `pnpm-lock.yaml` 恢复原样；`packageResult.kind` 按退出方式与输出对失败运行分类，`bundle` 给出完成的运行新增的包。`listBundles` 携带每个组合包的一句话简介（包的 `description`）、其 patch 声明的行及其存活条目，以及它覆盖的内置行；它列出 profile 自己的组合包、安装提供的组合包，以及被选中却没有组合包 patch 的名字（作为 `not-bundle` 问题），未选中的普通依赖不列出。本 fork 尚无经审核的安装自有可选提供清单，因此即使安装附带的组合包未启用，`optional` 仍为 false；Desktop 的 `required:false` Copilot 获取计划另由启动器拥有，不在这里推断。每个完成的操作都会发出 `plugin-manager/changed`；在管理器之外应用的一代 patch（HMR 监视到 CLI 或手工编辑后）不发通知，页面要到下一次读取才知道。
...
安全挂载后，`plugin_manager` 工具才可列出插件条目与组合包，并执行影响整个 profile 的改动。结果包含保存状态变化、应用状态和包管理诊断。管理操作不会向 Agent 注入消息。
