...
When `ctx.fs.sandboxMode` reports confinement, write/edit advertise `sandbox_permissions` and `justification` and resolve genuinely wider retries through `ctx.approval`. A same or narrower target keeps the standing mode without approval, so a redundant request never lowers the operation's authority. The policy owner contributes capability-neutral standing policy; the tool results retain operation-specific denial and retry guidance.
