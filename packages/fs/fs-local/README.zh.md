...
- **版本 token 依赖文件系统元数据**：它们组合设备、inode、大小和纳秒级 mtime，并在 Windows 上使用 birth time、在其他平台使用 change time。Windows 排除 change time，因为 NTFS 可能在读取时推进它；如果存储层在重写时保留所有选中的事实，仍可能绕过陈旧防护。
