...
任务分类器审计记录通过 `callId` 配对请求与结果；分类器位置按其请求顺序排列，而不是结果提交顺序。仅凭这些记录无法确定它们相对对话或压缩调用的分发顺序。成功、JSON/schema 无效、达到 token 上限和非文本结果会回放其保留分片，包括推理与用量。提供方失败的诊断已被隐藏时，回放会在原有前缀后追加固定的安全错误结束分片：该诊断是合成内容，并非提供方原始错误字节。输出超限、缺失结束分片、超时、取消与分发前失败的结果需要显式覆盖。位置补丁会先替换无法直接回放的调用位置，然后加载器检查剩余条目是否均可执行；格式错误、重复或未配对的审计记录仍会被拒绝。分类器请求不会到达在线提供方。

只有混合调用顺序没有歧义时，才允许按位置派生或打补丁。分类器请求若位于某个对话 step 开始与后续 Assistant settlement 之间，回放会拒绝：对话可能已经分发，也可能仍在等待准备完成。`step/start` 只是保守下界，绝不是分发时间戳；header、重试标记与分片时间都无法提供缺失的逐调用起点。混合日志缺少 step 下界、含有任何空的普通 settlement，或分类器请求之后出现带标记的本地压缩摘要时，也必须替换整个脚本。空 settlement 并不能证明没有发生分发。这些保守拒绝也可能涵盖保留记录无法确定顺序的串行执行。

这些情形应使用裸 `ReplayEntry[]` 伴随文件，显式给出已知调用顺序；`{ patches }` 无法修复未知索引。完整脚本替换仍会校验分类器审计字段与配对关系。只有主 Session 支持覆盖伴随文件，因此父级覆盖无法补救有歧义的子级脚本。不会推断或新增持久调用起点字段，也不会引入按用途分隔的队列。
