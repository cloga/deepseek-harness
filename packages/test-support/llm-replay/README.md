...
Task-classifier audits pair requests and results by `callId`; classifier positions follow their request order, not settlement order. This alone does not establish dispatch order relative to conversation or compaction calls. Successful, invalid-JSON/schema, max-token and non-text outcomes replay their retained chunks, including reasoning and usage. A redacted provider failure replays its prefix followed by a fixed safe error finish: that diagnostic is synthetic, not the original provider error bytes. Output-limit, missing-finish, timeout, cancellation and pre-dispatch failure outcomes require an explicit override. Positional patches replace refused call slots before the loader checks that every remaining entry is executable; malformed, duplicate or unpaired audits still reject. No classifier request reaches a live provider.

Mixed-call order must be unambiguous before positional derivation or patches are allowed. If a classifier request falls between a conversation step's start and a later Assistant settlement, replay refuses: the conversation might already have dispatched or might still be awaiting preparation. `step/start` is only a conservative lower bound, never a dispatch timestamp; headers, retry markers and chunk times do not supply the missing per-call start. Missing step bounds, any empty ordinary settlement in a mixed log, or a marked local compaction summary following a classifier request also require a whole-script replacement. An empty settlement does not prove that no dispatch occurred. These conservative refusals can include sequential executions whose order the retained records cannot establish.

Use a bare `ReplayEntry[]` sidecar with explicitly known call order for those cases; `{ patches }` cannot repair unknown indexes. Whole-script replacement still validates classifier audit fields and correlation. Only the primary Session supports an override sidecar, so a parent override cannot rescue an ambiguous child script. No durable call-start field or separate purpose queue is inferred or introduced.
...
- **Derivation requires retained output that reproduces the recorded outcome** — ordinary loop streams, marked local compaction outputs and supported classifier audits are derivable; lossy classifier outcomes, pre-chunk throws, cancel/hang and unmarked external summarizer calls require `replay.override.json`. Replacement and patch forms affect only the primary session; child scripts still derive from their logs.
