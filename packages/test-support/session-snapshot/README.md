...
### ACP configuration input

An ACP input script can select an advertised session configuration value with `{ "op": "setConfigOption", "configId": "model", "value": "<advertised model value>" }` after `newSession`. The harness sends the captured session id, `configId`, and `value` through `session/set_config_option` and awaits its response before the next input step. A missing session or a rejected update fails the scenario; the harness does not substitute another value. This lets a scenario change the selected model before a later prompt or command without fabricating a model turn.
