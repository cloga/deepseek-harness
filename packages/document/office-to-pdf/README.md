...
> Fork integration status: the alpha2 branch selects this official provider and its Client preview. The published Desktop is unchanged; the bundled Windows engine, ASAR-unpacked bytes, and real conversion still require same-source installer acceptance.
...
The provider depends on the independently published [`@deepseek-ai/libreoffice-kit`](https://www.npmjs.com/package/@deepseek-ai/libreoffice-kit/v/0.0.1) npm API at kit version `0.0.1`. Application packaging selects the matching native package declared in the kit’s `optionalDependencies`, or WASM when no native package is declared for that target. A missing declared native engine rejects packaging without selecting WASM. The independently published kit owns engine bytes; the Desktop installer must verify and unpack its declared target package. It never asks the user for system LibreOffice or downloads an engine at runtime.
...
The provider's owning queue, cache, and cancellation tests cover bounded admission and shared cancellation; this fork does not import upstream implemented-status notes as release evidence.
