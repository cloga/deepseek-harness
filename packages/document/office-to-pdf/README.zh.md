...
> fork 集成状态：alpha2 分支现已选用官方 provider 与 Client 预览；已发布 Desktop 不变。随包 Windows 原生引擎、ASAR 外资源字节和真实转换仍需通过同源安装器验收。
...
此 provider 依赖独立发布的 [`@deepseek-ai/libreoffice-kit`](https://www.npmjs.com/package/@deepseek-ai/libreoffice-kit/v/0.0.1) npm API，kit 版本为 `0.0.1`。应用打包选择 kit 的 `optionalDependencies` 中声明的匹配原生包；目标没有声明原生包时选择 WASM。已声明的原生引擎缺失时拒绝打包，不会选择 WASM。独立发布的 kit 拥有引擎字节；Desktop 安装器必须验证并在 ASAR 外保留目标原生包，不要求用户另装系统 LibreOffice，也不会在运行时下载引擎。
...
本包的队列、缓存和取消测试覆盖有界准入及共享取消；本 fork 不把上游的「已实现」Agent Note 当成此分支的发布验收证据。
