...
`registerLearningWeights(provider)` 安装一个归属 Fiber 的同步本地查询。只有高置信度的新主任务才会调用它；返回权重必须保留所有候选和硬性规则，只能在捕获的幅度上限内下调一个符合权限与质量底线的候选权重。提供方收到的是分离的路由／配置事实，而不是任务文本或实时 Session。提供方缺失、返回格式错误、抛错或异步返回时，均保留普通基础策略。上层所有者必须独立批准证据与活动版本；此接口本身不证明任务成功，也不会自动启用学习。
