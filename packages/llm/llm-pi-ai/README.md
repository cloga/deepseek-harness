...
`[text]` is the absence of a declaration rather than a guess at the endpoint, which is why the fallback here is conservative where the capacity fallbacks are merely plausible. Discovery adopts image input only when optional gateway metadata explicitly reports `capabilities.supports.vision: true` or a non-null `capabilities.limits.vision` object; false or absent metadata contributes no `input` and preserves this fallback. The two wrong answers do not cost the same: the harness refuses an image before it is attached when a model's modalities omit one, so under-claiming costs a refusal naming the model, while over-claiming admits an image the provider then rejects mid-turn — after the message is durable, which leaves the session repeating a request that cannot succeed.
...
A standard listing may disclose only an id; `context_window`/`context_length` and `max_output_tokens`/`max_tokens` are read when a gateway supplies them. Copilot-style metadata adds stricter facts without becoming required: an explicit false `model_picker_enabled`, `policy.state: disabled`, a non-`chat` `capabilities.type`, `capabilities.supports.tool_calls: false`, or a `supported_endpoints` list that excludes the selected OpenAI protocol removes that entry; absent fields leave it discoverable. `capabilities.limits.max_context_window_tokens` and `max_output_tokens` outrank their flat aliases, recognized `capabilities.supports.reasoning_effort` values become identity-mapped `reasoningEfforts` (`none` maps to the Harness `off` key), and `capabilities.supports.vision: true` or a non-null `capabilities.limits.vision` object contributes `input: [text, image]`. False or absent vision metadata contributes no `input`, preserving the text-only fallback. Entries without a usable id are skipped rather than failing the whole listing. The reply is read under a four-megabyte ceiling enforced on the bytes actually received — the endpoint is a URL the user typed, so a declared length is checked first but never trusted as the bound. An unreachable endpoint, a refused credential, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` and a message naming the endpoint and, for a 401 or 403 alone, the credential. Cancellation during the body read surfaces as `ABORTED`, like a cancellation before the request went out.
