...
`[text]` is the absence of a declaration rather than a guess at the endpoint, which is why the fallback here is conservative where the capacity fallbacks are merely plausible. Standard OpenAI-compatible listings disclose no modalities; optional gateway capability metadata can explicitly add image input during endpoint interrogation, while false or absent vision metadata leaves this fallback in force. The two wrong answers do not cost the same: the harness refuses an image before it is attached when a model's modalities omit one, while over-claiming admits an image the provider then rejects mid-turn after the message is durable.
...
Most listings disclose an id and nothing else; `context_window`/`context_length` and `max_output_tokens`/`max_tokens` are read when a gateway supplies them. A gateway that reports `capabilities.supports.vision: true` or a non-null `capabilities.limits.vision` object contributes `input: [text, image]`; false or absent metadata contributes no `input`, so profile resolution keeps its text-only fallback. Entries without a usable id are skipped rather than failing the whole listing. The reply is read under a four-megabyte ceiling enforced on the bytes actually received — the endpoint is a URL the user typed, so a declared length is checked first but never trusted as the bound. An unreachable endpoint, a refused credential, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` and a message naming the endpoint and, for a 401 or 403 alone, the credential. Cancellation during the body read surfaces as `ABORTED`, like a cancellation before the request went out.
...
- **A route's catalog never refreshes itself** — endpoint interrogation is an explicit edit action whose candidates are written only after the user adopts them, so a model list is only as current as its last edit.
...
- **A modality declaration is not verified** — discovery trusts optional gateway capability metadata rather than sending an image probe, so incorrect metadata or a hand-written `image` declaration is refused by the provider after prompt admission. The durable image remains in history and the same misdeclared model can fail again. Switching to a text-only model remains possible because the shared LLM runtime projects image references into stable text for that exact request.
