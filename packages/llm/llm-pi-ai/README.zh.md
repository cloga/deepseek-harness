...
`[text]` 是「尚未声明」，而不是对端点的猜测——这也是为什么这里的回退值取保守值，而两个容量回退值只是取一个说得过去的值。标准 OpenAI 兼容列表不公布模态；可选的网关能力元数据可以在端点询问时明确加入图片输入，而 `vision` 为 false 或缺席时继续采用该回退值。两种猜错的代价并不对等：模态中不含图片时，Harness 会在图片被附加之前就拒绝；而多声明会接纳一张图片、再由提供方在消息已持久化后拒绝。
...
- **路由的 catalog 不会自我刷新**：端点询问是显式编辑动作，候选只有在用户采纳后才会写入，因此模型列表的新鲜度只到最近一次编辑为止。
...
- **模态声明不经验证**：发现过程信任可选的网关能力元数据，而不会发送图片探测，因此错误元数据或手写的 `image` 声明会在 prompt 准入后被提供方拒绝。持久图片会留在历史中，同一个错误声明的模型可能再次失败。系统仍允许切换到纯文本模型，因为共享 LLM 运行时会在该次请求中把图片引用投影为稳定文本。
