...
When the composition provides `ctx.sessionProjections`, token-meter registers three projection units. `tokenUsage` carries the durable log's observed conversation and classifier `uncachedInputTokens`, `outputTokens`, `cacheReadTokens`, and `cacheWriteTokens`. A final assistant-message sample replaces streaming usage from the same attempt; `llm/retry-started` ends that replacement scope, so a retry in the same step contributes another billed attempt. `contextPressure` carries optional `pressureTokens` (the newest conversation provider-reported prompt size), optional `projectedTokens` (what the next request's prompt would cost), and optional `contextWindow` from the newest `request/context` record. `contextBreakdown` carries heuristic `systemTokens`, `toolsTokens`, and `messageTokens` — the context's composition, not its provider-billed size. Unloading the plugin removes all three keys.

`model/routing-result.usage` adds observed classifier usage to the total regardless of success, failure, or cancellation; its embedded stream is not counted again. Routing events never replace or clear the Assistant usage slot and never supply conversation context pressure. The optional `tokenUsage.routing` view appears after the first routing audit and exposes the classifier-only four-bucket subtotal, already included in the totals, plus `startedCalls`, `settledCalls`, and `usageReportedCalls`. A started call without settlement has unknown usage; a settlement without a report also remains unknown. The counters distinguish these cases from a measured zero without inventing tokens, prices, or savings.
...
`contextBreakdown` retains plain-JSON `{ seq, heuristicTokens, system }` entries in surface order and reuses the measurement plan/commit fold. Its state and surface transitions are O(current retained surface), not O(1) and not O(total historical log); replaced entries and message bodies are not retained. State version 5 invalidates checkpoints priced with offload metadata. `contextPressure` remains the scalar shadow-price consumer: replacements without adjacent claims contribute zero delta. The usage fold retains one Assistant last-sample slot because legal logs never report usage for an earlier step after a later step reports usage. Routing accounting retains scalar counters and one routing-event sequence watermark, not a growing call registry. Reapplying an already folded routing event is idempotent; producers must append one request and at most one terminal result per unique call id. This fold does not validate malformed logs that repeat a call's settlement at a different sequence. Token-usage state version 3 invalidates older checkpoints that omit routing overhead; context-pressure state is unchanged.
...
- **Unreported auxiliary usage is unknown** — routing totals sum only observed reports. The started, settled, and reported-call counters expose incomplete accounting; neither classifier failure nor cancellation proves zero provider cost.
