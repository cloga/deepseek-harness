...
调用 `installModelSelection()` 的入口还提供同步的 `model-selection/query` 监听器。使用 `readModelSelection(agent.ctx, agent)`，可为轮次外维护读取独立的已配置选择快照，而不组装提示词、运行异步解析或消费待应用的选择。该函数使用原始所有者对象作为作用域键；载荷只承诺该身份，不承诺完整的 Agent。没有选择时返回 `undefined`；尚未解析的策略只暴露其已配置后备值，不预测下一次解析的路由。查询不会改变正在执行的步骤已经捕获的选择。

当组装上下文携带 Agent 时，该辅助函数会在下游组装监听器之前，分发 Agent 作用域的异步 `model-selection/resolve` waterfall（瀑布式事件）。载荷携带该 Agent、在任何 await 之前捕获的独立已配置选择，以及可选的组装取消信号。通过 `next()` 委托时会得到捕获的选择，除非另一解析器替换它。没有 Agent 的诊断组装即使带有作用域，也会跳过解析。

只有成功且未取消的下游组装，才会发布用于提示词变量、请求路由和切换通知的已解析选择。解析器失败、下游组装失败或取消都会保留此前已组装的值。解析绝不覆盖可变的已配置选择：并发手动修改在下一次组装时应用，而不会使当前步骤的提示词与路由分裂。这个扩展点本身不实现 Auto 路由策略。
...
通过 `agent.ctx` 进行的注册可以遮蔽提示词段或工具，也可以在未发布 setup 期间安装仅适用于该 agent 的拦截器，因此一个 agent 看到的提示词与工具集会与其邻居不同。模型选择会在异步解析前复制已配置的提供方／模型／强度，再把成功组装的已解析路由用于该步骤的提示词变量和请求。并发的已配置选择修改等待下一次组装；没有 Agent 的诊断组装不会运行解析器。
