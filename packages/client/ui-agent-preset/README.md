...
When the roster carries the self-referential `cordis` preset, its dashed add-card stays disabled until the picker is enabled. Creator requests a fresh Session with `cordis` bound in the creation request, without applying Creator to the current blank or staging it for whichever Session arrives next. The chip's Creator cue is visual only. While creation is pending, the button is busy and rejects duplicate activation. The action closes Settings only when that fresh Session is still the committed main binding and the Host reports `cordis`; a failed, superseded, or obsolete section action does not close the panel.

The button includes localized guidance to choose a Workspace before invoking Creator. With no target Workspace, the action clears the main selection without creating a Session, leaves Settings open, and retains no deferred Creator intent. Choose a Workspace, then invoke Creator again. Ordinary chip choices and Settings default synchronization are unchanged.
