...
名单携带自指的 `cordis` preset 时，其虚线添加卡在选择器开启前保持禁用。Creator 请求新的 Session，并在创建请求中绑定 `cordis`；它不会把 Creator 应用到当前空白会话，也不会为接下来到达的任意 Session 暂存 Creator。chip 的 Creator 提示仅有视觉作用。创建尚未完成时，按钮显示忙碌状态并拒绝重复激活。只有该新 Session 仍为已提交的主 binding 且 Host 报告 `cordis` 时，该动作才关闭 Settings；失败、已被替代或所属分区已失效的动作不会关闭面板。

按钮附带本地化指引，说明调用 Creator 前应先选择 Workspace。没有目标 Workspace 时，该动作清除主选择但不创建 Session，保持 Settings 打开，也不保留延后的 Creator 意图。选择 Workspace 后，再次调用 Creator。普通 chip 选择和 Settings 默认值同步保持不变。
