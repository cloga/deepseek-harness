...
describe('shared composer dock layout ownership', () => {
  ...
  it('owns shared spacing once and removes an empty dock from layout', () => {
    ...
    expect(dock).toContain('align-items: center;')
    expect(dock).toContain('min-width: 0;')
    expect(dock).toContain('max-width: 100%;')
    expect(dock).toContain('padding-top: 4px;')
    expect(dock).not.toMatch(/(?:^|\n)\s*(?:width|padding|margin):/u)
    expect(dock).not.toContain('--dsh-composer-side-clearance')
    ...
    expect(composer).not.toContain('.root:has([data-composer-stats])')
    expect(composer.match(/^\.dock \{/gmu)).toHaveLength(1)
