...
describe('shared composer dock layout ownership', () => {
  ...
  it('owns shared spacing once and removes an empty dock from layout', () => {
    ...
    expect(rule(composer, '.dock:empty')).toContain('display: none;')
    expect(rule(composer, '.root')).toContain('padding: 0 var(--dsh-composer-side-clearance) 8px;')
    expect(rule(composer, '.root:has([data-composer-stats])')).toContain('padding-bottom: 4px;')
