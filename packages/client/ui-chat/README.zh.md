...
Chat 会在历史前插与 renderer 重新挂载时恢复语义锚点。没有读者移动的贴底滚动事件会立即更新跟随归属，避免后续布局变化使其底部位置失效。读者移动即使位于跟随阈值内，也保持待处理直到采样周期或 `scrollend`，防止布局增长抵消小幅滚动操作。读者跟随底部时，`ResizeObserver` 追随新的底部，并且无需读取行几何就选中最后一个已加载轮次。在已经打开的视图中，新的用户输入、steering 或本地提交追加可以在较早的读者采样结束前返回底部，但带锚点的历史前插除外。首次打开、已保存位置恢复、带锚点的前插以及普通流式／布局跟随仍保留采样保护；之后的读者移动仍可退出跟随。读者离开底部后，高度变化会保持顶部位置，再由阅读线几何选择活跃轮次。轮次导航预览位于 Markdown 代码块粘性头栏上方，而导航外框始终处于 composer 上方的 transcript 区域内。
