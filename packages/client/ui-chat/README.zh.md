...
会话统计在 `conversation.composer.dock` 中使用按内容宽度排列、可换行的分组。InputBar 拥有共享行和外部间距，因此空间足够时，后续公开 dock 贡献紧随原生时间与 token/缓存命中 pill。每个原生 pill 保留自己的按钮、可访问标签和锚定弹层；窄视口允许分组换行，不改变记账或弹层状态。
...
Chat 会在历史前插与 renderer 重新挂载时恢复语义锚点。没有读者移动的贴底滚动事件会立即更新跟随归属，避免后续布局变化使其底部位置失效。读者移动即使位于跟随阈值内，也保持待处理直到采样周期或 `scrollend`，防止布局增长抵消小幅滚动操作。读者跟随底部时，`ResizeObserver` 追随新的底部，并且无需读取行几何就选中最后一个已加载轮次；在已打开的视图中，新的自身用户消息、steering 或本地提交追加可在较早的读者采样完成前回到底部，但正在应用带锚点的历史前插时除外。首次打开、已保存位置恢复、带锚点的前插和普通流式／布局跟随仍保留采样保护；随后的读者移动仍可退出跟随。读者离开底部后，高度变化会保持顶部位置，再由阅读线几何选择活跃轮次。轮次导航预览位于 Markdown 代码块粘性头栏上方，而导航外框始终处于 composer 上方的 transcript 区域内。
