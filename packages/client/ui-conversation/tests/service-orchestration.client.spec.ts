...
describe('ConversationController', () => {
  it('aggregates retained shells without React mounts and preserves other scopes during disposal', async () => {
    const b = await bench()
    try {
      await b.runtime.sessions.add({ id: 's2', session: { prompt: b.prompt } })
      const other = b.runtime.sessions.retain('s2' as SessionId)
      await other.ready
      const second = b.hub.shellFor(b.runtime.sessions.binding('s2')!)
      b.shell.setDraft('hidden retained draft')
      second.setDraft('another retained draft')
      expect(b.hub.restartSafety.getSnapshot().hasDraft).toBe(true)
      second.setDraft('')
      expect(b.hub.restartSafety.getSnapshot().hasDraft).toBe(true)
      other.release()
      ...
      expect(b.hub.restartSafety.getSnapshot().hasDraft).toBe(true)
      b.shell.setDraft('')
      expect(b.hub.restartSafety.getSnapshot()).toEqual({ hasDraft: false, attachmentCount: 0, submitting: false })
