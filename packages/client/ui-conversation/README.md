...
The existing physical dock is a centered, wrapping row below the card. `conversation.composer.dock` entries follow their public list order; the shell owns outer spacing, compact entries size to content, and a full-row entry can request full width. An empty dock consumes no space, including when its sole public `data-slot` outlet is empty. Text-only entries, other controls, crash markers and a ContextMeter sibling remain nonempty. Public-slot eligibility is restricted to the ordinary composer with input and Session identity; Hero and no-Session states may still display ContextMeter in the same below-card dock. Native statistics retain their bottom-clearance marker.
