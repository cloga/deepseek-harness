...
import type { InputRestartSafety } from './input/facade.ts'
...
export function apply(ctx: Context, config: Config = Config({})): void {
  ...
  ctx.effect(() => {
    const product = (globalThis as typeof globalThis & {
      dshDesktop?: { updates?: { reportImpact?(impact: InputRestartSafety): void } }
    }).dshDesktop
    const publish = (): void => { product?.updates?.reportImpact?.(inputHub.restartSafety.getSnapshot()) }
    const off = inputHub.restartSafety.subscribe(publish)
    publish()
    return () => {
      off()
      // A disposed owner cannot attest the replacement document's input state.
      product?.updates?.reportImpact?.({ hasDraft: false, attachmentCount: 0, submitting: true })
    }
  }, 'ui-conversation: retained input restart safety')
