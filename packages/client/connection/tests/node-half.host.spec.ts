...
describe('connection node half', () => {
  it('admits only authenticated API traffic to an effect-owned waterfall and withdraws it on disposal', async () => {
    ...
    try {
      ...
      await routes[0]!.handler(fakeRequest({ host: 'untrusted.example', cookie }), foreign.response)
      expect(foreign.state.status).toBe(403)
      const crossSite = fakeResponse()
      await routes[0]!.handler(fakeRequest({ host: 'localhost', cookie, origin: 'https://untrusted.example' }), crossSite.response)
  ...
  it('awaits delegated response streaming before releasing an admitted request', async () => {
    const { ctx, routes, connection, dispose } = await mounted()
