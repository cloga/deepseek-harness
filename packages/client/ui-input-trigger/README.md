...
During query refinement, previous rows and their highlight remain visible, but Enter and Tab consume the gesture without picking until the highlighted source is ready. The listbox exposes the current query as `data-trigger-query` and sets `aria-busy` while any source is pending; options expose their own `data-source` and `data-source-status`, so one source's readiness does not depend on unrelated groups.
