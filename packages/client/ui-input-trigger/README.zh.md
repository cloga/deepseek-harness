...
查询细化期间，原有行及其高亮保持可见，但高亮项所属来源就绪前，Enter 和 Tab 只消费手势，不选取候选项。listbox 通过 `data-trigger-query` 暴露当前查询，并在任一来源仍待完成时设置 `aria-busy`；各选项通过自己的 `data-source` 和 `data-source-status` 暴露所属来源及其状态，因此判断某一来源是否就绪无需等待无关分组。
