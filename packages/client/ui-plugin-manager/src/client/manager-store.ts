...
export type ManagerNotice =
  ...
  | { readonly kind: 'prepared'; readonly transactionId: string; readonly seq: number }
...
export interface InstallState {
  ...
  /** Prepared privately, not installed or activated in the running profile. */
  ...
  private async read(): Promise<void> {
    try {
      do {
        ...
        const [bundles, plugins, pending] = await Promise.all([
          this.ctx.remote.pluginManager.listBundles(),
          this.ctx.remote.pluginManager.listPlugins(),
          this.ctx.remote.pluginManager.listPendingPackageChanges(),
        ])
        if (generation !== this.generation) return
        if (!bundles.ok || !plugins.ok || !pending.ok) {
          this.patch({ status: 'error' })
          continue
        }
        this.patch({
  ...
  private async startInstall(subject: InstallSubject, approvedBuilds?: readonly string[]): Promise<void> {
    ...
    if (!result.ok) {
      ...
      this.patchInstall({
        ...
        installed: result.value.application === 'prepared' ? null : result.value.bundle ?? null,
  ...
  private applied(answer: Answer<ChangeResult>, packageName: string): void {
    ...
    switch (result.application) {
      ...
      case 'cancelled':
        ...
        return
      case 'prepared':
        if (result.prepared === undefined) throw new RemoteAnswerError('Prepared package change has no transaction identity')
        this.patch({ notice: { kind: 'prepared', transactionId: result.prepared.transactionId, seq: ++this.noticeSeq } })
        return
