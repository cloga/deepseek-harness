...
export const zh = {
  ...
  empty: '还没有安装任何插件。',
  addPlugin: '添加插件',
  verifiedReleaseTitle: '安装或升级已验证 Release',
  verifiedReleaseLabel: 'Release 描述符 JSON',
  verifiedReleasePlaceholder: '粘贴发布者提供的完整 githubRelease 描述符',
  verifiedReleaseHelp: '粘贴发布者提供的完整 JSON 描述符。Desktop 会验证发布身份、制品及校验值；此输入尚未通过验证。',
  verifiedReleaseOwnership: '需要支持独立暂存的 Desktop。仅可替换后端确认归属的插件；不会覆盖运行时包或无归属的用户选择。候选插件在暂存副本中默认禁用；暂存不会改变当前插件的启用状态。',
  verifiedReleaseInterrupt: '暂存不会修改当前插件。请在 Desktop 应用菜单中查看暂存更改，保存草稿后单独确认激活；重启 Host 可能中断运行中的任务。',
  verifiedReleaseRun: '验证并暂存',
  verifiedReleaseInvalidJson: '请输入有效的 JSON 描述符；原始输入已保留。',
  verifiedReleaseWrongType: '描述符必须是 type 为 githubRelease 的 JSON 对象。',
  verifiedReleaseMissingPrepared: 'Host 未返回已暂存的事务。未确认激活；请检查暂存更改后再继续。',
  restartNotice: '更改将在下次启动生效',
  preparedTitle: '已暂存，尚未激活',
  preparedSelectionTargets: '组合包选择：{names}',
  discardPrepared: '丢弃暂存更改',
  preparedNotice: '事务 {id} 已在独立目录暂存。当前插件未改变；需在 Desktop 中单独确认并通过重启安全检查后才能激活。',
  ...
  installCancelled: '已取消安装请求，下载的文件可能保留。当前启用状态请查看已安装列表。',
...
export const en = {
  ...
  verifiedReleaseTitle: 'Install or upgrade verified Release',
  verifiedReleaseLabel: 'Release descriptor JSON',
  verifiedReleasePlaceholder: 'Paste the publisher’s complete githubRelease descriptor',
  verifiedReleaseHelp: 'Paste the publisher’s complete JSON descriptor. Desktop verifies the release identity, artifact and checksums; this input is not yet verified.',
  verifiedReleaseOwnership: 'Requires Desktop with private package staging. Only plugins whose existing ownership the backend verifies may be replaced; runtime packages and unowned user selections are protected. The prepared candidate defaults to disabled; preparation does not change the currently active plugin’s enablement.',
  verifiedReleaseInterrupt: 'Preparation leaves active plugins unchanged. Review staged changes in Desktop’s application menu and save drafts before separately confirming activation; restarting the Host may interrupt running tasks.',
  verifiedReleaseRun: 'Verify and prepare',
  verifiedReleaseInvalidJson: 'Enter a valid JSON descriptor. Your original text has been kept.',
  verifiedReleaseWrongType: 'The descriptor must be a JSON object with type githubRelease.',
  verifiedReleaseMissingPrepared: 'The Host did not return a prepared transaction. Activation is unconfirmed; inspect pending changes before continuing.',
  restartNotice: 'The change takes effect at the next start',
  preparedTitle: 'Prepared, not activated',
  preparedSelectionTargets: 'Bundle selection: {names}',
  discardPrepared: 'Discard prepared change',
  preparedNotice: 'Transaction {id} is staged privately. Active plugins are unchanged; activation requires separate Desktop confirmation and restart-safety checks.',
  ...
  installCancelled: 'Installation request cancelled; downloaded files may remain. Check the installed list for current enablement.',
