...
> Integration status in this fork: source only. The shipped Web/Desktop roster does not mount this page or its Manager Remote. Staged Desktop writes are refused until the native transaction owner and verified source/receipt UI are connected; never submit credential-bearing signed URLs through this page.

## Summary

After an authorized Host and roster are integrated, the **Plugins** entry can manage installed profile bundles; this fork currently has no declared installation-owned optional offers. Switch bundles and their rows on and off, install a bundle after the Host has read what the spec names, watch pnpm's output, stop a run, and enable what it added. Uninstalling asks for confirmation. A plugin that registers a configuration page is edited here, on its own page; Settings keeps the read-only inventory.
...
Once a reviewed Host/Client roster mounts it, select **Plugins** in the sidebar. The page reads the inventory and the bundles through `api-remotes` when first opened; a Host without a managed profile shows the page as unavailable. **Official** comes first for separately reviewed optional offerings (none declared in this fork) and plugins that registered a configuration page; **Installed** lists the bundles the profile holds. Cards are listed by name, so switching a bundle on or off does not move its card. A dependency without a bundle patch is not a plugin and is not listed unless the profile selects it, in which case it carries a problem tag. Global configuration remains in the Settings **Plugins** section.
