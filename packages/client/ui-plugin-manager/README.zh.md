...
在[由启动器暂存包操作的 profile](../../boot/plugin-manager/README.zh.md#launcher-owned-staging) 中，成功结果改为显示**已暂存，尚未激活**及事务 UUID。该包不会作为已安装项出现，也不提供**立即启用**。页面在刷新或重新连接时恢复待处理的暂存记录；**丢弃暂存更改**只移除暂存内容，不删除已激活的组合包。待处理的组合包选择记录显示全部实际包名，而非虚构的单个目标；该展示既不代表已启用，也不构成命令来源的激活授权。激活及重启确认由 Desktop shell 负责，不由本页执行。

**安装或升级已验证 Release**在支持启动器暂存的 Desktop 中接受发布者提供的完整 `githubRelease` JSON 描述符。页面检查 JSON 和来源类型，随后直接提交对象而不执行字符串 spec 检查；Host 验证发布身份、字节、校验值及替换归属。准备好的候选配置中插件保持禁用，当前运行的插件不变。输入无效、请求被拒绝或取消得到确认时，描述符保留供编辑。成功必须返回已暂存事务，并遵循上面的独立激活流程，绝不显示**立即启用**或声称已有激活 receipt。
...
- **没有版本选择器**——页面不发现注册表或 GitHub Release 版本。已验证 Release 的安装或升级需要发布者提供的完整描述符及由启动器暂存的 Desktop；普通 spec 保留原有检查规则。
