...
在[由启动器暂存包操作的 profile](../../boot/plugin-manager/README.zh.md#launcher-owned-staging) 中，成功结果改为显示**已暂存，尚未激活**及事务 UUID。该包不会作为已安装项出现，也不提供**立即启用**。页面在刷新或重新连接时恢复待处理的暂存记录；**丢弃暂存更改**只移除暂存内容，不删除已激活的组合包。激活及重启确认由 Desktop shell 负责，不由本页执行。
