...
function bench(overrides: Partial<Record<string, ReturnType<typeof vi.fn>>> = {}) {
  ...
  const plugins = {
    ...
    listPendingPackageChanges: vi.fn(() => Promise.resolve(ok([]))),
    cancelPendingPackageChange: vi.fn(() => Promise.resolve(ok(undefined))),
...
it('recovers prepared transactions independently of installed inventory and discards only their UUID', async () => {
  const pending = { transactionId: '11111111-1111-4111-8111-111111111111', state: 'prepared' as const,
    packageName: 'prepared-plugin', baseFingerprint: 'a'.repeat(64), health: 'pending' as const }
  const listPendingPackageChanges = vi.fn().mockResolvedValueOnce(ok([pending])).mockResolvedValue(ok([]))
  const b = bench({ listPendingPackageChanges })
  try {
    b.face.ensure()
    await vi.waitFor(() => { expect(b.state().pendingPackages).toEqual([pending]) })
    expect(b.state().packages.some(pkg => pkg.name === pending.packageName)).toBe(false)
    b.face.cancelPrepared?.(pending.transactionId)
