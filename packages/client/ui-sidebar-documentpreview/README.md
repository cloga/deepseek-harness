...
> Fork integration status: the alpha2 branch mounts the official Office renderer, but the published Desktop is unchanged. The frozen Client bundle and installed Windows LibreOffice-kit engine need the same-source renderer and conversion acceptance before release.
