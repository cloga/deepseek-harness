...
it('identifies the managed Release channel without claiming installer bytes are downloaded', async () => {
  const f = fixture()
  try {
    f.view.rerender(<f.Indicator dictionary={en} />)
    await f.emit({ phase: 'ready', version: '1.0.1.cloga.1', mode: 'github-release-managed' })
    const review = screen.getByRole('button', { name: 'Review installer' })
    fireEvent.focus(review)
    expect((await screen.findByRole('tooltip')).textContent).toContain('cloga verified GitHub Release')
...
it.each(['preserved-user-choice', 'pending'] as const)('shows an idle baseline notice without hiding update actions: %s', async status => {
  ...
  try {
    f.view.rerender(<f.Indicator dictionary={en} />)
    await f.emit({ phase: 'idle', baseline })
    const notice = screen.getByRole('button')
    expect(notice.getAttribute('data-error')).toBe('true')
    fireEvent.focus(notice)
    expect((await screen.findByRole('tooltip')).textContent).toContain('fixture-provider')
    fireEvent.click(notice)
    await act(async () => {})
    expect(f.open).toHaveBeenCalledOnce()
    await f.emit({ phase: 'downloading', percent: 58, baseline })
    const busy = screen.getByRole('button', { name: '58%…' })
    expect(busy.getAttribute('aria-disabled')).toBe('true')
    expect(busy.hasAttribute('data-error')).toBe(false)
    fireEvent.click(busy)
    expect(f.open).toHaveBeenCalledOnce()
    await f.emit({ phase: 'ready', mode: 'github-release-managed', baseline })
    fireEvent.click(screen.getByRole('button', { name: 'Review installer' }))
    await act(async () => {})
    expect(f.open).toHaveBeenCalledTimes(2)
    await f.emit({ phase: 'idle' })
    expect(screen.queryByRole('button')).toBeNull()
  } finally { f.view.unmount(); f.status.resolve({ phase: 'idle' }) }
})
