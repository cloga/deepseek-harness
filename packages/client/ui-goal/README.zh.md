...
与 `ui-conversation` 及 goal 领域包一起挂载本插件；只要会话存在目标，条带就会作为 composer 上下文堆栈的第二张卡片出现（位于 Todo 之后、Queue 之前）。已 armed 的 active goal 提供暂停动作；active-but-disarmed 或 paused 的 goal 提供恢复；编辑会打开一个高度受限、可垂直调整大小的三行字段，其中 Enter 插入换行，Ctrl/Cmd+Enter 保存，Escape 取消；纯空白目标仍然无效。清除会移除目标，并在投影追上之前抑制条带。
