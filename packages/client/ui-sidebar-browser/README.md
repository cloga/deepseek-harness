...
This fork imports the official package as source only: the shipped Web and Desktop compositions do not mount it yet. After Web/Slot integration and packaged iframe, popup, and permission acceptance, the right-Sidebar guide can open **Browser**, accept an HTTP(S) URL, or route an HTTP(S) Assistant Markdown link. A host name without a scheme becomes HTTPS. Public and loopback targets use the same default sandbox. Each activation creates another Browser tab.
