...
本 fork 仅导入官方包源码，正式 Web 与 Desktop composition 尚未挂载。完成 Web／Slot 接线和打包后的 iframe、弹窗与权限验收后，右侧 Sidebar guide 才可打开 **浏览器**、输入 HTTP(S) URL，或转接 Assistant Markdown 中的 HTTP(S) 链接。不带 scheme 的主机名会补全为 HTTPS。公共目标与 loopback 目标使用相同的默认 sandbox。每次操作都会创建一个新的 Browser tab。
