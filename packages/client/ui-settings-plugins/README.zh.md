...
**默认模型规则**使用 Host 的动态模型目录，将直接父 Agent 的精确提供方和模型组合映射到子 Agent 默认模型。规则仅适用于支持模型配置且原本会继承父模型的子 Agent；列表为空时保留该继承行为。提供方的固定默认模型或自有模型控制保持不变，匹配的规则也不会阻止其正常委派。规则仅在未来创建时匹配一次：不会连续套用规则，不会改变已有或恢复的 Agent，也不会选择主 Session 模型。原生已授权的路由与推理强度选择，以及调用方配置的 LLM 选项优先。添加默认规则绝不会启用独立的模型选择权限。

规则也适用于兼容的分叉提供方。更换分叉子 Agent 的模型可能失去继承前缀的缓存复用，需要重新处理历史，并不保证降低成本。这是用户显式设定的默认规则，与 Core 有意禁用的 AI 分叉模型选择相互独立，不会开启该权限。

可添加或移除规则行，并为两端选择提供方与模型；未完成的行或重复的父模型组合会阻止保存。当前目录中缺失的已存 ID 仍可见且可移除，不会被替换。编辑器区分未保存、保存中和已保存设置；保存并不代表模型可用或已被实际使用。Host schema 中没有 `modelRules` 时，规则编辑被禁用并显示不支持的提示；值缺失时按空列表显示。

保存 Subagent 卡片时会校验所有分区。已修改的限制与 `modelRules` 在同一次带 revision 栅栏的 `subagent` mutation 中提交，仅包含修改过的叶字段。仅保存规则绝不写入限制或模型选择权限。`subagent-model-selection` mutation 仍然独立：任一命名空间失败时，其草稿保留供修改，重试仅写入剩余草稿。目录刷新保留草稿；连接重置会丢弃 Host 专属草稿并忽略延迟结算。只要部署提供任一命名空间，页面就会出现，且仅显示可用分区。
