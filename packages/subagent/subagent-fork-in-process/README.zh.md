...
`dsh-subagent-fork-in-process` 是一个进程内 subagent 后端：它以父级已完成的对话轮次作为每个子 agent 的初始内容——子 agent 能看到所有已完成轮次，但看不到进行中的轮次，因此后续工作可以在对话基础上继续，而无需重复提供对话内容。委派工具以 `fork` 提供方名称找到它，它与 spawn 共用原生创建生命周期，但在自身路由选择中排除任务感知 Auto。当子任务延续当前对话时选择它；当子 agent 必须独立运行时选择 spawn。初始内容是 fork 时的一次性快照：此后父级记录的任何内容都不会到达子 agent。
...
当子 agent 需要对话的已完成轮次时——后续分析、审查、延续——选择 fork。当子 agent 应全新开始时选择 spawn；当子 agent 不能共享本进程时选择进程外后端。初始内容传递对话历史，而非授予权限：子 agent 获得全新工具作用域，由共享原生服务单独应用已捕获的权限状态。

### 初始内容边界

初始内容止于父级最后一个已完成的轮次。subagent 启动时，父级当前的工具调用轮次仍在进行，因此该进行中的轮次绝不会被包含；在第一个已完成轮次之前，初始内容为空，但 fork 仍保留自身的模型路由策略，而不会启用全新 spawn 的 Auto。
...
后端提供可信的原生 fork 标记，并计算父级日志的已配平已完成轮次前缀，并把它作为子 agent 的会话初始内容交给共享进程内驱动器。由于实际序号等于数组下标，前缀始终是自序号零开始的合法初始内容；驱动器记录其长度，使结果读取器不会把作为初始内容的父级消息误认为子 agent 输出。
...
子 agent 先接收由父级已配平的已完成轮次构成的前缀，再逐字接收新的任务内容。配置的 persona 会在子 agent 的全新作用域中遮蔽提示词文本；工具限制会过滤其全局协议 schema、可执行工具查找与 PTC mode SDK 绑定，但不影响独立指导内容。父级的实时工具视图不会被复制；已捕获权限与未来委派偏好单独应用，并不启用 fork 自身的 Auto 模式。可选的结构化输出请求会添加仅属于子 agent 的约定；父级当前进行中的轮次会被排除。
...
- **fork 排除自身任务感知 Auto 选择**——兼容父路由仍是默认值，而显式配置或用户编写的精确父模型规则可以改变模型或推理强度并影响缓存复用。随附 fork 工具省略面向模型的路由字段。独立捕获的委派偏好仍可指导后续获准的全新 spawn，而不重新路由 fork；见[共享原生选择约定](../subagent/README.zh.md#native-model-selection)。
