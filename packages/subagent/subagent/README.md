...
Native spawn and fork share one creation-time selection chain. Explicit or configured provider, model, or reasoning effort, including provider-owned route defaults, takes precedence. Otherwise, an exact user-authored parent-model rule wins; an authorized fresh spawn may then use task-aware Auto; remaining cases inherit compatible direct-parent values. Matching reads the parent's actual latest request route, not a pending picker selection; creation options supply values before its first request. The configured output-token cap is captured separately. External backends retain their existing selection behavior.

The optional `subagent` settings namespace exposes `modelRules`, defaulting to no rules. Each entry maps an exact `parent` provider/model pair to a `child` pair; duplicate parent pairs are rejected. Composition configuration works without a settings service, and settings changes affect later creations rather than an in-flight capture or an existing child. A matched target is validated before creation; an unavailable rule target does not fall through to Auto.

Task-aware Auto is separate from permission Auto review. It requires a captured delegation preference, a native fresh-spawn provider, and the parent's captured `subagent/model-selection-policy` route authorization. The service owns that unchanged first-record-wins projection; tool consumers still capture consent for fresh Sessions, and absence remains disabled. Auto intersects its candidate pool with the authorized exact routes and validates live model/effort support. Missing consent leaves ordinary inheritance available; an admitted empty pool or an unauthorized/unavailable conservative candidate fails closed. Explicit request denials are never recovered through Auto. A deny-only invocation flag disables that invocation's Auto choice, not exact user rules or later delegation preferences.

Every native child receives any captured delegation preference separately from its own fixed model and from its copied route authorization, including children selected by an explicit request, parent rule or fork inheritance. Preference alone grants no model access and never enables Auto for the child's own conversation. A later fresh grandchild repeats the chain against its actual direct parent and its own captured authorization. Fork itself does not use task-aware Auto; explicit configuration or a user rule can still change its route and affect cache reuse. A continuable child's descriptor fixes its model and effort for cold resume without reclassification.
...
| [`src/child-agent.ts`](src/child-agent.ts) | Child composition, captured-option merging, delegated policy, depth helpers |
| [`src/native-model-selection.ts`](src/native-model-selection.ts) | Shared capture, resolution, authorization, topology checks and creation evidence |
| [`src/model-rules.ts`](src/model-rules.ts) | Exact parent-route rule validation and matching |
| [`src/model-selection-policy.ts`](src/model-selection-policy.ts) | Shared exact-route authorization types and validation |
| [`src/model-selection-state.ts`](src/model-selection-state.ts) | First-record-wins permission projection and child seeding |
...
A request is validated against the provider's advertised capabilities, a durable descriptor is snapshotted, and the provider builds the child. Both in-process providers advertise `agentOptions` and use the [native selection chain](#native-model-selection). The registry captures parent options, route authorization and permission state before asynchronous selection; the driver receives complete resolved options and never merges them with a later parent state. The permission snapshot is separate from model routing: a parent using permission Auto review or Full access gives the child the same `permission/preset` identity, while the existing sandbox override and approval-policy pin continue to apply. Recording both identities prevents an older same-bundle fork value from winning. Permission Auto then reviews every supported child call independently: ordinary project-local work is low risk and allowed, medium-risk work requires explicit action, exact-target and scope authorization from the existing creation prompt or an authenticated human/direct-parent message, without conflicting human limits, while high-risk work is always denied. The reviewer derives that context from `parentSession` and existing messages; permission review adds no parent call metadata or review receipt. Model-selection evidence is recorded separately. A route change without an explicit effort clears the inherited route-owned effort so the selected model resolves its default. DSH SDK also advertises `agentOptions` but runs a separate child runtime, so it does not inherit permission Auto review; ACP, Codex, and Claude Code likewise retain their own permission systems after the parent delegation call passes review. On success the run is published and ownership transfers to the caller; on failure the provider rolls back every unpublished resource. The result carries the child's final output, an optional structured value, a stop reason, and an optional safe diagnostic.

Native setup records `subagent/model-selection` after any inherited marker and before tool composition; it describes the resolved creation choice, not a dispatched model call. `request/header` owns actual request use. Separate `model/delegation-auto` data carries future delegation preference, and the existing route-policy event carries authorization. An Auto-selected combination whose own effort is absent never re-inherits the parent's effort, even on the same provider/model. Registry unload cancels and joins native startup; its combined creation signal does not replace the original caller signal governing a returned one-shot run.

### Continuable flow

For fresh native creation, the manager validates non-route composition and reserves child identity and parent ownership before classifier work. It resolves complete model options, builds the final descriptor, creates the child Agent, installs an Activation, and submits the prompt. Cold resume uses the saved descriptor without running the selection chain again. Model-authored messages cross one parent/child edge through fixed Steer scheduling; browser human prompts choose Queue or best-effort Steer through an internal adapter, while other host protocols may retain Queue for distinct turns. A Session queue command admits a live subagent-owned Agent only from its own continuable descriptor. Settlement waits for Agent activity to finish, an empty Inbox, and no owned children, then flushes final Session state with admission open. Under the child lock, the manager revalidates the wake generation, Session sequence, Inbox, and owned children; the synchronous task entry of `Agent.runMaintenance()` claims the idle phase and closes the private subagent Inbox in the same JavaScript turn before handle disposal. An absent direct-child Activation cold-resumes from the persisted session. When a resident Activation settles, the manager tells the child's direct parent in the parent's own turn stream.
...
- [Task-aware routing design proposal](../../../.agents/notes/proposed/feature/2026-09-21-task-aware-auto-routing.md) — routing rationale and parent-rule integration context.
...
The child runs with the resolved model and effort. Eligible task-aware Auto may send the isolated child task to a separate bounded classifier; its prompt and audit behavior belong to [model-routing](../../llm/model-routing/README.md). Creation evidence and delegation preferences are log-only and do not become conversation messages.

#### Token effect

Classifier usage is recorded under the direct parent and can remain chargeable even if later child creation fails. Explicit route/effort selections, provider-owned route defaults, matching parent rules and ordinary inheritance do not call the classifier. The child pays its own conversation-model tokens; no currency savings are inferred from route names.

#### KV Cache effect

Selection does not rewrite the parent's conversation prefix. A fresh spawn has its own cache; a fork route or effort change may affect reuse of its inherited prefix.
