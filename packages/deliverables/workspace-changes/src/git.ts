...
export async function gitlinkPaths(git: GitRunner, workspace: GitWorkspace, signal: AbortSignal): Promise<Set<string>> {
  ...
  if (result.truncated) throw new Error('git ls-files output exceeded the configured cap')
...
export async function ignoredPaths(
  ...
  if (result.truncated) throw new Error('git check-ignore output exceeded the configured cap')
