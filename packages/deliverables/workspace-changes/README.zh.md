...
本插件汇总每个顶层轮次改动了哪些文件、每个文件的行数，并提供每个所列文件在轮次开始与结束时的内容对比。比较轮次开始和结束时的 git 工作树快照；文件工具编辑的每个文件在首次编辑之前和轮次结束时各复制一份整文件，以此覆盖 git 覆盖不到的文件。没有仓库或没有 git 时，只列文件工具的编辑。Session 日志只收到一条写明轮号的 `workspace/changes` 事件；摘要和对比留在 Host 上，直到 Session 释放。待客户端及同 Session 授权路由接入后，Web 卡片才可渲染它们。

> 本 fork 当前仅导入源码；正式组合尚未自动执行 git 快照，也没有挂载改动文件卡片。
...
本 fork 的正式 Web／Host 尚未挂载此源码包。启用前需审查 git 过滤器副作用、存储保留和同 Session 路由授权；隔离组合另需 `subprocess` 能力与 Host 上的 git 可执行文件：
...
- [Web 产出物](../../client/ui-deliverables/README.zh.md)——当前文件工具产出展示；本 fork 尚未挂载轮次级改动卡片。
