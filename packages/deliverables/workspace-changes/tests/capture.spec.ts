...
describe('captureFile', () => {
  it('stores text once by content, reads no more than the cap, and classifies absent, oversized, binary, and non-regular paths', async () => {
    ...
    expect(basename(a.file)).toMatch(/^[0-9a-f]{64}$/u)
    expect(await readFile(a.file, 'utf8')).toBe('one\ntwo\n')
    expect(await readdir(store)).toHaveLength(1)
    await writeFile(join(root, 'different.txt'), 'three\n')
    const different = await captureFile(join(root, 'different.txt'), store, 16)
    if (different?.kind !== 'file') throw new Error('expected another stored copy')
    expect(different.file).not.toBe(a.file)
    expect(await readFile(different.file, 'utf8')).toBe('three\n')
    expect(await readdir(store)).toHaveLength(2)
