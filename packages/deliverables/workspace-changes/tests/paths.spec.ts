...
import { canonicalPath, compareDisplay, displayPathOf, durablePathOf, isInside, isTemporaryPath, temporaryRoots } from '../src/paths.ts'
...
describe('durablePathOf', () => {
  it('keeps legitimate dotdot-prefixed file names relative without accepting parent traversal', () => {
    const named = join(cwd, '..notes')
    const outside = join(cwd, '..', 'other')
    expect(isInside(cwd, named)).toBe(true)
    expect(displayPathOf(named, cwd, root, home)).toBe('..notes')
    expect(durablePathOf(named, cwd)).toBe('..notes')
    expect(isInside(cwd, outside)).toBe(false)
    expect(durablePathOf(outside, cwd)).toBe(outside)
  })
