...
This fork has not mounted this source package in its shipped Web or Host. Before enabling it, review Git filter side effects, storage retention, and same-Session route authorization. An isolated composition additionally needs the `subprocess` capability and a git executable on the Host:
...
- [Web deliverables](../../client/ui-deliverables/README.md) — the current file-tool delivery display; turn-level change cards are not mounted in this fork yet.
