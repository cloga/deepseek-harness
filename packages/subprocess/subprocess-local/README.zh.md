...
把提供方与它的消费方挂载在同一组合中，并完全按子进程服务的规定启动进程；本包只决定这些进程在宿主机上如何运行。在 Windows 上，非终端子进程与 `taskkill` 辅助进程会隐藏窗口，因此后台操作不会抢占焦点。这包括私有 Windows Job runner 及其原生目标进程。遵循进程启动可见性设置的 GUI 窗口也会被隐藏，但不会阻止程序随后主动显示窗口。终端会话仍使用独立的 PTY 路径。
