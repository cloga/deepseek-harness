...
把提供方与它的消费方挂载在同一组合中，并完全按子进程服务的规定启动进程；本包只决定这些进程在宿主机上如何运行。在 Windows 上，非终端子进程与 `taskkill` 辅助进程请求隐藏初始窗口，包括私有 Windows Job runner 及其原生目标进程。启动可见性不改变标准流或 Job 归属，不隐藏已有的父进程控制台，也不阻止程序随后主动显示窗口。仅凭这些标志不能确定整个桌面的可见性或焦点行为。终端会话仍使用独立的 PTY 路径。
