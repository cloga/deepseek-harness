...
- **The credential scrub is a name heuristic** — it matches `*KEY*`/`*PASSWORD*`/`*SECRET*`/`*TOKEN*`, with a separate [temporary Git configuration group rule](../subprocess/README.md). Other differently named secrets (for example `*PASSPHRASE*`) pass through, and a whitelist for over-scrubbed variables is noted future work.
