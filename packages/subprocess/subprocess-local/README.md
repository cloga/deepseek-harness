...
Mount the provider beside its consumers and start processes exactly as the subprocess service specifies; this package decides only how those processes run on the host. On Windows, non-terminal children and `taskkill` helpers start with their windows hidden so background operations do not take focus. This includes the private Windows Job runner and its native target. It also hides GUI windows that honor the process startup visibility setting, but does not prevent a program from explicitly showing a later window. Terminal sessions keep their separate PTY path.
