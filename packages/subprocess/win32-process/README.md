...
- **Hidden initial windows** — ordinary and restricted-token children receive `STARTF_USESHOWWINDOW` with `SW_HIDE` alongside their standard handles. Console inheritance and process creation flags remain unchanged; callers still own output and Job cleanup.
