...
在 `cloga/deepseek-harness` 中，CI 默认使用标准 GitHub 托管 Linux 与 Windows 运行器，并降低 worker 并发；显式且符合使用条件的 failover 选择仍优先。其他仓库保留其配置的运行器池。检查项、覆盖率阈值和超时预算不变。Linux 隔离环境准备从 Launchpad 官方归档下载固定版本的 Ubuntu bubblewrap 包，解包前验证 SHA-256，并要求功能探测通过。
