...
flowchart LR
  ...
  svc_modelRouting["ctx.modelRouting<br/>Captured task-aware model routing"]
  ...
  svc_modelRouting --> pkg_api_session_controller
  svc_modelRouting --> pkg_subagent
...
| `ctx.modelRouting` | `core` | [`model-routing`](../packages/llm/model-routing) | - | [`api-session-controller`](../packages/api/session-controller), [`subagent`](../packages/subagent/subagent) | - | 捕获 Session Auto 策略，并在提示组装前解析提供方、模型和推理强度。原生注册表负责权限交集；可选的学习权重不能替换硬性策略或当前任务。 |
