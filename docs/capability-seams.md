...
| `ctx.profilePackageTransactions` | `core` | [`app-boot`](../packages/boot/app-boot) | - | [`plugin-manager`](../packages/boot/plugin-manager) | - | Desktop Host supplies fixed-profile preparation and cancellation; Electron owns acquisition and separate activation. |
