...
Ordinary in-place installation treats omitted `InstallBundleOptions.enabled` as true; false skips selecting the bundle layer. Launcher-owned staging preserves selection for an already declared package when `enabled` is omitted, enables a new bundle by default, and applies an explicit selection only to the candidate graph. For ordinary in-place installation, `approvedBuilds` grants persistent script permission to the supplied pending package names before installation. Launcher-owned staging can reject unsupported script approvals; it never applies those permissions to the active profile before activation.
