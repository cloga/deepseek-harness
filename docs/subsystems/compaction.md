...
`changed`, `summary`, and `summary-truncated` close and persist the failed attempt without a summary replacement; image omissions recorded during recovery remain effective. `summary-truncated` identifies a summary that reached its output token cap rather than a generic summarization or shrink failure. `commit` may follow partial mutation; `persistence` means the in-memory bracket closed but its flush failed. Cancellation remains separate and throws the exact abort reason after required cleanup.
