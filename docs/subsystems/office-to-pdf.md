...
The external [`@deepseek-ai/libreoffice-kit`](https://www.npmjs.com/package/@deepseek-ai/libreoffice-kit/v/0.0.1) Node API selects its precompiled engines. The kit has its own version and release workflow. Application builds install its published npm packages. Application packaging requires the target’s declared native engine, or Node WASM when the kit declares no native engine for that target. The Desktop installer must verify the declared target package and its files without using system LibreOffice or a runtime download. Invalid metadata, missing required assets, and conversion errors reject without switching engines. Conversion uses disk input and output paths on the Host, with no browser conversion engine or font RPC.
