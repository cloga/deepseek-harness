...
import { lstatSync, mkdirSync, mkdtempSync, readFileSync, readdirSync, realpathSync, rmSync, symlinkSync, linkSync, writeFileSync } from 'node:fs'
...
function selfTest() {
  ...
  try {
    ...
    rmSync(link, { recursive: true }) // Node requires recursive removal for Windows junctions.
...
if (mode === 'self-test') {
  ...
  if (mode === 'before') {
    ...
    exclusiveWrite(output, 'before.json', receipt, MAX_RECEIPT)
    inventory(output, ['before.json'])
    console.log(`Bound source ${source} and two original golden inputs.`)
  } else {
    inventory(output, ['before.json'])
    ...
    inventory(output, finalFiles)
    console.log('Captured exactly two measured golden outputs and two bounded receipts; independent workflow and adoption audit still required.')
  }
}
