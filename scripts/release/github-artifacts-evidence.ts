...
async function successfulRun(api: GitHub, id: string, path: string, selection: Selection): Promise<Run> {
  ...
  if (String(run.id) !== id || !Number.isSafeInteger(run.run_attempt) || run.run_attempt < 1
    ...
    || (run.pull_requests.length > 0
      && !run.pull_requests.some(pr => pr.number === 81 && pr.head.sha === selection.reviewedHead && pr.base.ref === BASE))) {
...
export async function verifyEvidence(api: GitHub, selection: Selection, tree: string, artifactId: string): Promise<string> {
  ...
  const pr = requireValue(await api.json<Pull>('/pulls/81'), 'PR 81')
  if (pr.number !== 81 || !pr.merged || !pr.merged_at || pr.draft || pr.merge_commit_sha !== selection.mergedCommit
  ...
  const branch = requireValue(await api.json<BranchFacts>('/graphql', 'POST', { query:
    'query { repository(owner: "cloga", name: "deepseek-harness") { ref(qualifiedName: "refs/heads/review/issue-72-official-base") { branchProtectionRule { requiresApprovingReviews requiredApprovingReviewCount requiresStatusChecks requiredStatusCheckContexts requiredStatusChecks { context app { databaseId } } } } pullRequest(number: 81) { reviewDecision } } }',
  }), 'Branch requirements')
  ...
  const reviews = await api.list<Review>('/pulls/81/reviews')
