...
| Private Desktop slash-command bridge | Framework support is complete, but the bridge is absent: the exact target's commands registry and `command/run`/`command/done` events exist; its Desktop Host entry registers no Desktop plugin command | Reuse the official registry and Session lifecycle; retain the narrow exact-child adapter until official Desktop supplies equivalent durable flush, acknowledgement, native consent and live-Host cancellation ownership. |
| Launcher staging and pending records | Absent upstream: the exact target has neither app-boot `types.ts` nor `profile-package-transactions.ts`, and its PluginManager `ChangeResult` has no `prepared` state or field | Both the existing five-field stage result and the seven-field selection pending result are fork integration on alpha2, not official features. Keep ordinary stage unchanged and extend only pending queries; return to equivalent official support with an explicit data migration. |
| Atomic selection and guarded registry updates | Partial: official PluginManager and profile helpers provide ordinary file-locked in-place bundle selection, but not candidate-only atomic selection, journal-bound command authority or source/receipt-preserving native activation | Keep the shared official UI and bundle representation; retain the fork transaction owner until official behavior verifies equivalent under-lease target eligibility, artifact/ownership preservation, consent, admission, health and recovery. |
...
| Own-message visibility during pending scroll sampling | Partial: the official [ChatView](../../../../packages/client/ui-chat/src/client/chat/ChatView.tsx) defers its entire layout effect during a reader sample. An ordered source-callback counterexample loses the local submission's only force-follow observation when it is replaced by durable user and Assistant nodes before sampling settles; this does not establish the cause of a historical browser failure. | Retain a narrow own-append exception only for an already-open view without an anchored prepend. Keep ordinary streaming/layout, initial restoration, timer ownership and later reader gestures unchanged. Retire it when the official implementation passes equivalent ordered own-send, restoration, prepend, reader-input and real-browser coverage. |
...
- Ordinary Web Later preserves PREPARED for status/discard without live activation, successful active receipts or Host interruption. Pre-admission command cancellation discards only its own preparation after quiescence; an orphan command preparation cannot gain ordinary Web activation. Concurrent requests, stale fingerprints, process exit and restart recovery fail closed under the same lease; admitted activation retains its separate recovery rules.
