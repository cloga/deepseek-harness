...
Capability schema 2 contains only the fixed `cloga/deepseek-harness` owner, `dsh-desktop-v` tag prefix, `release.json` asset name, packaged sequence, minimum sequence, and one exact migration record. It does not accept a user-selected repository or URL.
