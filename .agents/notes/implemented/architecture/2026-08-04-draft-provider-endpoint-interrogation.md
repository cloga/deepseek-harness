...
- `LlmDiscoveredModel` makes every field but `id` optional, because a standard listing may disclose only an id. Richer gateways can add settings-ready capacities, reasoning-effort mappings, and input modalities without making those fields requirements for every OpenAI-compatible endpoint.
...
`packages/llm/llm/tests/topology.spec.ts` covers the registry: one offer per namespace, disposal with the fiber, detached normalization that drops duplicate and unusable ids without inventing metadata, and the `NO_DISCOVERY`/`INVALID_DISCOVERY` refusals. `packages/llm/llm-pi-ai/tests/discovery.spec.ts` drives the probe against local HTTP servers — minimal listings, flat capacities, Copilot nested limits, recognized reasoning efforts, vision true/false/absent metadata, every metadata filter including selected endpoints, a preserved deployment path, credential resolution, malformed rows and bodies, transport failures, cancellation, unsupported protocols, and both size-limit paths. `packages/host/apiproxy/tests/api-proxy-config.spec.ts` carries the richer candidate fields over the real proxy, and `packages/client/ui-settings-models/tests/provider-form.client.spec.tsx` proves adoption writes them into the profile while preserving an already-tuned row.
