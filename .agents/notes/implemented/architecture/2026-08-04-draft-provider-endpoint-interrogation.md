...
- `LlmDiscoveredModel` makes every field but `id` optional, because a standard listing may disclose only an id. Richer gateway fields, including accepted input modalities, are candidate metadata that a configuration surface may adopt into the adapter profile.
...
`packages/llm/llm/tests/topology.spec.ts` covers the registry: one offer per namespace, disposal with the fiber, normalization that drops duplicate and unusable ids, detached optional modalities, and the `NO_DISCOVERY`/`INVALID_DISCOVERY` refusals. `packages/llm/llm-pi-ai/tests/discovery.spec.ts` drives the probe against local HTTP servers — vision true, false, absent, and limits metadata; listings with and without disclosed capacities; a preserved deployment path; credentials; malformed rows and replies; cancellation; unsupported protocols; and both size-limit paths. `packages/host/apiproxy/tests/api-proxy-config.spec.ts` covers candidate modalities over the real RPC proxy, and the model-editor component test proves an adopted candidate writes those modalities into the provider profile.
