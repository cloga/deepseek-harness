...
The scanner stops retaining events at the first unparsable row, sequence gap, scalar overlap, or conflicting overlap but continues inspecting later complete records. A packed row may repeat an identical contiguous prefix already present in the log; the scanner compares those events, discards the duplicate prefix, and retains the row's unseen suffix. A later `turn/end` proves that a deferred issue lies in the committed region and rejects the log. The Zstandard reader also rejects any unresolved parse, sequence, overlap, or partial-record issue after all complete frames; only a structurally torn final frame may contribute a recoverable suffix. Complete records emitted from that torn frame pass through the same scanner and retain the existing repair offset and recovered-event semantics.
