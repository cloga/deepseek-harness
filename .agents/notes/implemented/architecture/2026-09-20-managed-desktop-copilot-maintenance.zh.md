...
仅升级 Copilot 的维护计划保留了 Core `0.1.6-alpha.1`，并选择不可变的 [Copilot `0.4.0-alpha.30`](https://github.com/cloga/dsh-github-copilot/releases/tag/v0.4.0-alpha.30)。alpha.2 Desktop 候选已纳入该插件锁定及其维护验收要求。Copilot alpha.30 同时支持 Core alpha.1 与 alpha.2，但这种兼容性并不能证明 Core alpha.2 Desktop 适配通过验收。
