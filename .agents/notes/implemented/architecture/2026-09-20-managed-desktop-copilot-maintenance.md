...
The Copilot-only maintenance plan retained Core `0.1.6-alpha.1` and selected immutable [Copilot `0.4.0-alpha.30`](https://github.com/cloga/dsh-github-copilot/releases/tag/v0.4.0-alpha.30). The alpha.2 Desktop candidate incorporates that plugin lock and its maintenance acceptance requirements. Copilot alpha.30 supports both Core alpha.1 and alpha.2, but that compatibility does not qualify the Core alpha.2 Desktop adaptation.
