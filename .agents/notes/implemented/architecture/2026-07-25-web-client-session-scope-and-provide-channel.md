...
- Ordinary Workspace connection reuses eligible blank members, but explicit New Session creates a fresh identity. Sequential explicit creation and independent clients can therefore leave multiple blanks in one Workspace; non-current blanks remain hidden. The [fresh-identity decision](../bug-fix/2026-09-19-explicit-new-session-fresh-identity.md) supersedes only the explicit New Session reuse policy; the scope and provisioning decisions remain active.

### connectWorkspace: ordinary Workspace selection

`uiWorkspace.connectWorkspace(workspaceId): Promise<SessionId>` belongs to `client/ui-workspace`, which reads both Workspace and Session Controllers:
...
- Explicit New Session uses `uiWorkspace.startSession`, not `connectWorkspace`. Its Workspace choice and creation semantics are defined in the [Workspace README](../../../../packages/client/ui-workspace/README.md).
...
- Blank-session state uses a derived bit, and the unified list projection shows only the current blank as `New Session`. Same-Workspace reuse avoids unnecessary creation during ordinary selection; it imposes no per-Workspace blank count limit on explicit New Session.
