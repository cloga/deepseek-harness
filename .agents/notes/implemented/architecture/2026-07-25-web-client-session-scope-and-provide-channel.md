...
- Blank residue requires no dedicated GC: after a refresh, blank Sessions remain eligible for ordinary same-Workspace reuse while they remain members; after a Host restart, never-appended blanks leave no disk trace. Explicit fresh starts and multi-tab races can leave multiple blanks in one Workspace. Only the main-view blank is visible; ordinary reuse does not delete the other identities.

### connectWorkspace: ordinary Workspace selection

`uiWorkspace.connectWorkspace(workspaceId): Promise<SessionId>` belongs to the UI navigation owner, which reads both Controllers without placing selection in either Controller snapshot. Explicit New Session uses the separate fresh-creation policy in [Explicit New Session creates a fresh identity](../bug-fix/2026-09-19-explicit-new-session-fresh-identity.md); ordinary selection retains these reuse rules:
...
- Explicit New Session chooses its supplied Workspace, then the main Session's Workspace, then the most recent ready Workspace, and creates a fresh identity through `startSession`. Recency compares each Workspace's newest Session `updatedAt`, falls back to Workspace `createdAt`, and keeps Host order on ties. Without a target, the action clears the `mainView` reference into the no-Session view and creates nothing; it retains no deferred creation intent.
...
- `session.create` accepts workspaceId/cwd as either-or, an optional creation-time `agentPreset`, and an optional caller-preallocated sessionId (a same-id same-cwd retry is idempotent; a different cwd reports `session-conflict`).
...
- Blank-session governance takes zero dedicated mechanisms: state rides one derived bit, visibility rides the unified list projection (only the current blank shows, as `New Session`), reclamation rides lazy persistence's existing contract (evaporation on restart), and ordinary selection retains same-Workspace reuse without limiting explicit fresh starts.
