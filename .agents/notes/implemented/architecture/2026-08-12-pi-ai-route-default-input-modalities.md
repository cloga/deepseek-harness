...
**Undeclared means `[text]`, and that is the absence of a declaration rather than a guess at the endpoint.** A minimal standard OpenAI listing reports no modalities. A gateway may publish Copilot-style `capabilities.supports.vision` or `capabilities.limits.vision`; endpoint interrogation adopts that explicit declaration, while an unannotated entry still reaches this conservative fallback. Under-claiming refuses the image before it is attached, names the model, and has a documented configuration remedy. Over-claiming admits and persists an image before the provider can reject it. Later requests to that same incorrectly declared route will encounter the image again, although the user can select a text-only model because request assembly projects durable images to placeholders.
...
**No configuration surface directly edits `input`.** It joins `compat`, `reasoningEfforts`, `thinkingBudgets`, and `headers` as a settings-document field, and the model-list editor stays a hand-written form over id, name, and the two capacities. The fetch action can adopt `input` and `reasoningEfforts` when the endpoint explicitly supplies them. The card carries fields it does not edit: its row patch spreads the stored row before applying changes, and adoption keeps an existing row over a rediscovered candidate, so both discovered and hand-written metadata survive later edits.
...
- **Require endpoint modality metadata** — the standard OpenAI listing does not define it, so requiring a Copilot-style extension would make ordinary compatible endpoints undiscoverable.
...
A vision model on a custom provider either arrives with `input: [text, image]` from explicit gateway discovery metadata or costs one line written in the settings document — one per model, or one at the route when every model it lists takes images. The three admission points then admit images on it and `read_image` works. A deployment whose endpoint and settings declare nothing keeps exactly the conservative text-only behavior.

The image-admission gate keeps its meaning everywhere, because every modality it reads is recorded by the installed catalog, explicitly published by the endpoint, or written by a person. Discovery never infers vision from an id or makes its metadata mandatory.
...
`packages/llm/llm-pi-ai/tests/discovery.spec.ts` proves Copilot-style support and limit metadata become image input while minimal unannotated entries stay discoverable, and `packages/client/ui-settings-models/tests/provider-form.client.spec.tsx` proves adoption preserves that metadata in the profile.
