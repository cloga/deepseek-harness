...
**No configuration surface edits `input` directly.** It joins `compat`, `reasoningEfforts`, `thinkingBudgets`, and `headers` as a settings-document field, and the model-list editor stays a hand-written form over id, name, and the two capacities. The editor preserves stored fields it does not edit and adopts `input` when endpoint discovery supplies it; an existing tuned row still wins over a rediscovered candidate.
...
- **Treat every listing as authoritative for modalities** — the standard OpenAI-compatible response reports none. Only explicit optional vision metadata is adopted; absence remains text-only.
...
A vision model on a custom provider costs one line, `input: [text, image]`, written in the settings document — or one line at the route when every model it lists takes images. Discovery can write that model field through the editor when optional gateway metadata declares vision support. The three admission points then admit images on it and `read_image` works. A deployment whose endpoint supplies no such metadata and that writes nothing keeps the text-only behavior.
...
`packages/llm/llm-pi-ai/tests/discovery.spec.ts` covers vision true, false, absent, and limits metadata. Registry, RPC, and model-editor tests pin detached propagation into the adopted profile.
