...
**Undeclared means `[text]`, and that is the absence of a declaration rather than a guess at the endpoint.** Standard OpenAI-compatible listings report no modalities. Compatible gateways may add optional nested capability metadata, and endpoint interrogation adopts image input only when that metadata explicitly reports vision support; false or absent metadata stays undeclared. The only safe floor is the modality every supported protocol certainly carries. Under-claiming refuses the image before it is attached and has a configuration remedy. Over-claiming admits and persists an image before the provider can reject it.
...
**No configuration surface edits `input` directly.** It joins `compat`, `reasoningEfforts`, `thinkingBudgets`, and `headers` as a settings-document field, and the model-list editor stays a hand-written form over id, name, and the two capacities. The editor preserves stored fields it does not edit and also adopts `input` when endpoint discovery supplies it; an existing tuned row still wins over a rediscovered candidate.
...
- **Rely only on endpoint metadata** — optional gateway extensions are useful when present but are not part of the standard listing, so they cannot replace the conservative fallback or explicit configuration.
...
A vision model on a custom provider carries `input: [text, image]` from explicit gateway discovery metadata when available; otherwise it costs one line in the settings document, or one route default when every listed model takes images. The three admission points then admit images on it and `read_image` works. A deployment whose listing supplies no vision metadata and whose settings write nothing keeps the text-only behavior.

The image-admission gate keeps its meaning everywhere, because every positive image capability it reads is recorded by the installed catalog, explicitly reported by gateway metadata, or written by a person. Absence never claims image support on a deployment's behalf.
...
`packages/llm/llm-pi-ai/tests/discovery.spec.ts` covers vision true, false, absent, and limits metadata. The LLM registry and RPC tests preserve and detach discovered `input`, and the model-editor component test proves profile adoption while keeping existing tuned rows authoritative.
