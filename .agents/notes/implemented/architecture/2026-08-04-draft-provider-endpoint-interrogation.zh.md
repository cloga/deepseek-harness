...
- `LlmDiscoveredModel` 除 `id` 外每个字段都可选，因为标准列表可以只公布 id。信息更丰富的网关可以补充可直接写入 settings 的容量和推理档位映射，而这些字段不会因此成为所有 OpenAI 兼容端点的要求。
