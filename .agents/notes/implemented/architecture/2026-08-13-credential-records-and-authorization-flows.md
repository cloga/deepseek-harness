...
`llm-pi-ai` covers the three translations against a real `$DSH_HOME` document — an api-key credential field by field, strict-JSON normalization of an OAuth credential including extension fields, a foreign plugin's record skipped by scope, and the write refusal without a credentials service — plus every `AuthEvent` and `AuthPrompt` member restated, with `Models.login()` mocked at the collection boundary since a real one opens a browser. The normalization tests also pin realm-neutral plain-record admission, detachment, undefined handling, and path-only rejection of non-JSON categories. Two real-composition tests boot the plugin with and without the authorization seam.
