...
扫描器在遇到第一条无法解析的记录、序列缺口、标量重叠或内容冲突的重叠后停止保留事件，但会继续检查后续完整记录。打包行可以重复日志中已存在且内容相同的连续前缀；扫描器会比较这些事件，丢弃重复前缀并保留该行中未见过的后缀。后续出现 `turn/end`，说明延后处理的问题位于已提交区域，系统会拒绝该日志。处理完所有完整帧后，如果仍存在未决的解析错误、序列错误、重叠错误或部分记录，Zstandard 读取器同样会拒绝日志；只有结构上撕裂的最终帧才能提供可恢复后缀。该撕裂帧产出的完整记录会经过同一扫描器，并保持既有修复偏移量与恢复事件语义。
