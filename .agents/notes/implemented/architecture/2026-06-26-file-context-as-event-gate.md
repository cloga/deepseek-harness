...
The local provider's version token combines device, inode, size, and nanosecond mtime with ctime on POSIX and birthtime on Windows. NTFS may advance ctime during a successful read, so using it there would make the recorded pre-read version immediately stale even though no writer changed the file. Atomic replacement still changes the file identity, and ordinary content rewrites change size or mtime.
