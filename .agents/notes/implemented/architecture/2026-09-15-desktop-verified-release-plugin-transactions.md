...
The [plugin retention decision](../bug-fix/2026-09-17-desktop-plugin-retention-and-lockfiles.md) supersedes the receipt-based deletion rules below. This note retains ownership of source acquisition, graph validation, and transactional activation.
...
The recovery release plan selects immutable `dsh-github-copilot@0.4.0-alpha.22`, whose packed authorization and Schemastery dependencies are required Host peers. React is a Client external, not a required Node peer or a second private runtime copy. Published artifact and historical shared-inventory checks do not establish new Desktop materialization or installed UI behavior. Models, account, discovery, device-code, and update persistence remain recovery rehearsal and release acceptance obligations.
