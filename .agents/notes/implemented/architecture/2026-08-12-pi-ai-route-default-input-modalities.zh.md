...
**未声明即 `[text]`，而这是「尚未声明」，不是对端点的猜测。** 最简标准 OpenAI 列表不报告模态。网关可以发布 Copilot 风格的 `capabilities.supports.vision` 或 `capabilities.limits.vision`；端点询问会采纳这份显式声明，未标注条目则仍落到保守回退值。少声明会在图片附加之前拒绝、点名模型，并给出有文档的配置补救方法。多声明会先接纳并持久化图片，再由提供方拒绝。之后对同一错误声明路由的请求还会再次遇到图片，但用户可以选择纯文本模型，因为请求组装会把持久图片投影为占位符。
...
**没有任何配置界面直接编辑 `input`。** 它和 `compat`、`reasoningEfforts`、`thinkingBudgets`、`headers` 一样是 settings 文档字段，而模型列表编辑器仍是一张只覆盖 id、名称和两个容量的手写表单。端点明确提供时，“获取模型列表”动作可以采纳 `input` 与 `reasoningEfforts`。该卡片会承载自己不编辑的字段：它的行 patch 会先展开已存储的行再应用改动，而采纳候选时已有行优先于重新发现的候选，因此发现所得与手写元数据都能在后续编辑中存活。
...
- **要求端点提供模态元数据** —— 标准 OpenAI 列表没有定义该字段，要求 Copilot 风格扩展会让普通兼容端点无法被发现。
- **从模型 id 推断**（`*-vision`、`*-vl`）—— 命名约定不是能力，网关可以随意改名。
- **继续拒绝，只改进诊断** —— 那条消息对状态的描述本来就准确，对补救办法则毫无用处；缺的正是补救办法。

## 影响

自定义提供方下的视觉模型要么经网关显式发现元数据直接获得 `input: [text, image]`，要么只需在 settings 文档里写一行——逐模型写，或当该路由列出的模型全都接受图片时在路由上写。这三个准入点随即接纳它上面的图片，`read_image` 也可用。端点与 settings 都未声明的部署保持保守的纯文本行为。

图片准入门禁在各处都保住了自己的意义，因为它读到的每一个模态，如今要么由已安装 catalog 记录，要么由端点显式发布，要么由人写下。发现不会根据 id 推断视觉能力，也不会把该元数据变成必填项。
...
`packages/llm/llm-pi-ai/tests/discovery.spec.ts` 证明 Copilot 风格的 supports 与 limits 元数据会成为图片输入，而未标注的最简条目仍可被发现；`packages/client/ui-settings-models/tests/provider-form.client.spec.tsx` 证明采纳会把这些元数据保留在 profile 中。
