...
**未声明即 `[text]`，而这是「尚未声明」，不是对端点的猜测。** 标准 OpenAI 兼容列表不报告模态。兼容网关可以增加可选的嵌套能力元数据，端点询问只在该元数据明确报告视觉支持时采纳图片输入；false 或缺失仍保持未声明。安全的底线是每个受支持协议都确定携带的模态。少声明会在图片附加之前拒绝，并提供配置补救方法；多声明会先接纳并持久化图片，再由提供方拒绝。
...
- **只依赖端点元数据** —— 可选网关扩展存在时很有用，但并非标准列表的一部分，因此不能取代保守回退值或显式配置。
- **从模型 id 推断**（`*-vision`、`*-vl`）—— 命名约定不是能力，网关可以随意改名。
- **继续拒绝，只改进诊断** —— 那条消息对状态的描述本来就准确，对补救办法则毫无用处；缺的正是补救办法。

## 影响

自定义提供方下的视觉模型在网关提供明确发现元数据时直接携带 `input: [text, image]`；否则仍可在 settings 文档里写一行，或在该路由所有模型都接受图片时写一个路由默认值。三个准入点随即接纳图片，`read_image` 也可用。列表不提供视觉元数据且 settings 不写任何声明的部署保持纯文本行为。

图片准入门禁在各处都保住了自己的意义，因为它读到的每一项图片肯定能力，要么由已安装 catalog 记录，要么由网关元数据明确报告，要么由人写下。缺失永远不会替部署宣称图片能力。
...
`packages/llm/llm-pi-ai/tests/discovery.spec.ts` 覆盖 vision 为 true、false、缺失与 limits 元数据。LLM 注册表与 RPC 测试保留并分离复制发现的 `input`；模型编辑器组件测试证明 profile 采纳，同时保留已有调优行的优先级。
