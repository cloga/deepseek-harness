...
**未声明即 `[text]`，而这是「尚未声明」，不是对端点的猜测。** 标准 OpenAI 兼容列表不报告模态，但可选网关元数据可以明确报告视觉支持。发现只会为 `capabilities.supports.vision: true` 或非空的 `capabilities.limits.vision` 对象采纳 `[text, image]`；false 或缺失元数据不产生声明。安全的底线是每个受支持协议都确定携带的模态。少声明会在图片附加之前拒绝、点名模型，并给出有文档的配置补救方法。多声明会先接纳并持久化图片，再由提供方拒绝。之后对同一错误声明路由的请求还会再次遇到图片，但用户可以选择纯文本模型，因为请求组装会把持久图片投影为占位符。
...
**没有任何配置界面直接编辑 `input`。** 它和 `compat`、`reasoningEfforts`、`thinkingBudgets`、`headers` 一样是 settings 文档字段，而模型列表编辑器仍是一张只覆盖 id、名称和两个容量的手写表单。编辑器会保留自己不编辑的已存字段，也会在端点发现提供 `input` 时采纳它；已有的调优行仍优先于重新发现的候选。
...
- **把每一份列表都视为模态权威** —— 标准 OpenAI 兼容响应不报告模态。只采纳明确的可选视觉元数据；缺失时继续采用纯文本。
- **从模型 id 推断**（`*-vision`、`*-vl`）—— 命名约定不是能力，网关可以随意改名。
- **继续拒绝，只改进诊断** —— 那条消息对状态的描述本来就准确，对补救办法则毫无用处；缺的正是补救办法。

## 影响

自定义提供方下的视觉模型只需在 settings 文档里写一行 `input: [text, image]`——或者当该路由列出的模型全都接受图片时，在路由上写一行。可选网关元数据声明视觉支持时，发现也能通过编辑器写入该模型字段。三个准入点随即接纳它上面的图片，`read_image` 也可用。端点未提供此类元数据且部署什么都不写时，继续保持纯文本行为。
...
`packages/llm/llm-pi-ai/tests/discovery.spec.ts` 覆盖视觉 true、false、缺失和 limits 元数据。注册表、RPC 与模型编辑器测试钉住向已采纳 profile 的脱离式传播。
