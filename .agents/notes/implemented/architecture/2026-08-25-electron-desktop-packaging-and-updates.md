...
Plugin mutation and recovery follow the [verified release transaction decision](2026-09-15-desktop-verified-release-plugin-transactions.md). The unsigned fork stream follows the [fork-owned Windows release decision](2026-09-15-fork-owned-windows-desktop-release-channel.md).
...
Desktop selects exactly one update implementation from packaged resources. A signed package with `app-update.yml` uses `electron-updater` and retains platform publisher validation. An unsigned cloga package without `app-update.yml` can carry `managed-update/capability.json` plus the standalone `managed-update/helper.mjs`; startup rejects a package that carries both configurations. Both implementations replace the complete Desktop release, so there is no dsh-only update operation or independent runtime compatibility range.

The managed capability fixes the source repository, Desktop tag prefix, manifest asset name, packaged sequence, and minimum sequence without carrying a mutable URL. A check lists that repository's GitHub Releases, requires immutable release state and a commit-pinned tag, verifies the manifest asset digest, and then validates the versioned manifest's canonical self-hash, source commit and tree, sequence, build inputs, fork identity, installer hashes, installed evidence, network and completion policies, and generic `desktopNativeVerifiedRelease` capability, source-schema, and receipt-schema compatibility. The manifest forbids automatic plugin provisioning. The packaged sequence prevents a deployed build from selecting itself; the durable completion sequence prevents rollback. The parser accepts the exact `cloga/dsh-windows-ops` release only as a sequence-zero migration when the source channel has no release. Renderer IPC contributes only bounded composer-impact fields and cannot select repositories, URLs, executables, paths, process ids, or arguments.

An install confirmation reports Host-owned running Sessions, queued messages, active jobs, and renderer-owned unsaved input, then repeats when that impact changes before installation begins. Electron creates a random operation directory, copies its bundled Node.js executable and standalone helper, writes one main-process-owned handoff, and waits for an acknowledgement containing the one-time token, selected manifest hash, and spawned helper process id. A missing, malformed, mismatched, or timed-out acknowledgement leaves Desktop running, writes a cancellation marker, terminates only the owned helper, and waits for its exit. A Host-stop failure rolls back updater-owned quit and cancels the acknowledged helper through the same path. The helper checks cancellation before acknowledgement, during process waiting, and before installer launch. It waits only for the handoff's Electron and Host process ids, applies a fixed timeout to each network request, rejects streamed bytes beyond the manifest size, downloads and revalidates the build receipt and installer, and writes a pending marker. PowerShell holds a non-writable handle while it rehashes the installer, checks the declared Authenticode state, and starts interactive NSIS with no arguments; the child environment excludes credential-shaped variables. The helper never kills another process or suppresses Windows warnings or UAC.

The next Desktop startup scans only its operation directory. A successful helper result is not completion: the running executable and `desktop-runtime.json` hashes, manifest sequence, and pending marker must all match the release. Desktop leaves installed plugins unchanged and writes the durable completion sequence after installed evidence succeeds. An acknowledged handoff without terminal state, interrupted operations, nonzero installer exits, malformed or conflicting records, and installed-evidence mismatches enter startup recovery instead of starting a new Host or reporting success. Windows Ops `Complete` remains the external recovery owner for the legacy migration and consumes the same manifest and receipt identities.
...
The cloga fork release workflow is the only unsigned publication path. Windows Ops selects one supported upstream baseline at a time, and the reviewed source plan records that baseline while advancing both semantic version and integer sequence. The workflow supplies the fixed fork identity and managed capability, verifies the standalone helper and absence of `app-update.yml`, and publishes installer, manifest, receipt, and checksums from a clean pinned Windows build. A protected release job is the only job with repository write permission; it publishes all assets before finalizing the draft and then requires GitHub immutable state, exact tag commit, and matching remote asset digests.
