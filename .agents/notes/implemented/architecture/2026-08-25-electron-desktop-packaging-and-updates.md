...
The managed capability fixes the source repository, Desktop tag prefix, manifest asset name, packaged sequence, and minimum sequence without carrying a mutable URL. A check lists that repository's GitHub Releases, requires immutable release state and a commit-pinned tag, verifies the manifest asset digest, and then validates the versioned manifest's canonical self-hash, source commit and tree, sequence, build inputs, fork identity, installer hashes, installed evidence, network and completion policies, and verified GitHub Release plugin source and receipt versions. The packaged sequence prevents a deployed build from selecting itself; the durable completion sequence prevents rollback. The parser accepts the exact `cloga/dsh-windows-ops` release only as a sequence-zero migration when the source channel has no release. Renderer IPC contributes only bounded composer-impact fields and cannot select repositories, URLs, executables, paths, process ids, or arguments.
...
The next Desktop startup scans only its operation directory. A successful helper result is not completion: the running executable and `desktop-runtime.json` hashes, manifest sequence, pending marker, and verified plugin-provision receipt must all match the release. Desktop performs the release-locked GitHub Release plugin transaction before starting the active Host and writes the durable completion sequence only after activation succeeds. An acknowledged handoff without terminal state, interrupted operations, nonzero installer exits, malformed or conflicting records, installed-evidence mismatches, and plugin failures enter startup recovery instead of starting a new Host or reporting success. Windows Ops `Complete` remains the external recovery owner and consumes the same manifest and receipt identities.
