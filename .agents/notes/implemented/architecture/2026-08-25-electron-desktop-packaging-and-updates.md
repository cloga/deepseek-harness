...
An install confirmation reports Host-owned running Sessions, queued messages, active jobs, and renderer-owned unsaved input. Electron creates a random operation directory, copies its bundled Node.js executable and standalone helper, writes one main-process-owned handoff, and waits for an acknowledgement containing the one-time token and selected manifest hash. A missing, malformed, mismatched, or timed-out acknowledgement leaves Desktop running. After acknowledgement, Electron transfers quit ownership to the updater path, gracefully stops only its owned Host, and exits. The helper waits only for the handoff's Electron and Host process ids, downloads and revalidates the build receipt and installer, writes a pending marker, verifies the staged installer again, removes credential-shaped environment variables from child processes, and starts interactive NSIS with no arguments. It never kills a process or suppresses Windows warnings or UAC.
