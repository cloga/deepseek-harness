...
Desktop selects exactly one update implementation from packaged resources. A signed package with `app-update.yml` uses `electron-updater` and retains platform publisher validation. A Windows Ops package without `app-update.yml` can carry `managed-update/capability.json` plus the standalone `managed-update/helper.mjs`; startup rejects a package that carries both configurations. Both implementations replace the complete Desktop release, so there is no dsh-only update operation or independent runtime compatibility range.

The managed capability locks one immutable `cloga/deepseek-harness` GitHub Release manifest by its versioned HTTPS URL, canonical JSON SHA-256, minimum sequence, expected Desktop version, and expected source commit. The parser accepts an optional `cloga/dsh-windows-ops` manifest only as a sequence-zero migration. A check validates the exact manifest fields, canonical self-hash, source repository, tag, commit, sequence, build receipt, installer hashes, installed evidence, and verified GitHub Release plugin source. Manual redirects remain credential-free HTTPS and stay on GitHub release asset hosts. Renderer IPC contributes only bounded composer-impact fields and cannot select URLs, executables, paths, process ids, or arguments.
