...
The cloga fork release workflow is the only unsigned publication path. Its reviewed plan advances both semantic version and integer sequence, supplies the fixed fork identity and managed capability, verifies the standalone helper and absence of `app-update.yml`, and publishes installer, manifest, receipt, and checksums from a clean pinned Windows build. A protected release job is the only job with repository write permission; it publishes all assets before finalizing the draft and then requires GitHub immutable state, exact tag commit, and matching remote asset digests.
