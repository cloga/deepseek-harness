...
恢复 release plan 选择不可变的 `dsh-github-copilot@0.4.0-alpha.21`，其打包的 authorization 与 Schemastery 依赖均为必需的 Host peer。已发布制品和历史共享清单检查不证明新 Desktop 的物化或已安装 UI 行为。Models、account、discovery、device-code 与更新持久性仍属于恢复 rehearsal 和 release 的验收义务。
