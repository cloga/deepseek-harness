...
经过评审的 release plan 选择不可变的 `dsh-github-copilot@0.4.0-alpha.33`。其搜索 UI 声明路由 Remote namespace 依赖，打包的 authorization 与 Schemastery 依赖仍为必需的 Host peer。React 是 Client external，而非必需的 Node peer 或第二份私有运行时副本。打包设置验收要求只读显示当前工作区、无需模型前置条件的仅提供方 primary 与 fallback 控件、不包含已移除 compatibility disclosure 的登出 Manage，以及不输出登出额度界面的必需账户用量 capability。它不发起 OAuth、验证导航、模型调用、搜索或实时 quota access。Cron 不进入 release plan，继续归用户所有。已发布制品、peer 范围和历史共享清单检查不证明新 Desktop 的物化或已安装 UI 行为。Models、account、discovery、device-code 与更新持久性仍属于 rehearsal 和 release 的验收义务；[托管 Copilot 维护决策](2026-09-20-managed-desktop-copilot-maintenance.zh.md)拥有官方优先迁移条件和发布证据。
