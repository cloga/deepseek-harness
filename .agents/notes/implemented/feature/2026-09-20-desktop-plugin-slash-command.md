...
The [independent packaged command fixture](../../../../apps/desktop/tests/fixtures/desktop-plugin-command-smoke.ts) creates one synthetic Session and exercises registry discovery, list, invalid Release rejection, and native cancellation through supported RPCs. Inspection probes use its private environment and temporary directory. The application enters a nonbreakaway Windows Job before resuming; cleanup requires both root exit and zero active Job processes. Unknown spawn ownership or abnormal helper completion retains the private home instead of reporting quiescence. Native window discovery includes hidden windows and requires a unique root matching the verified process and actual CDP page title; visibility and enumeration order are not identity. Only that revalidated HWND may be shown, restored or closed, without disabling the application sandbox. Pure validators reject incomplete audit records and late successful responses; these tests do not replace hosted Windows native-dialog acceptance. The separate Copilot smoke remains unchanged and creates no Sessions. Fixed main-process phase messages distinguish settlement, cancellation, and native confirmation invocation without recording command arguments. Confirmation candidates come from bounded Win32 enumeration because UIA desktop children can omit owned modal windows; canonical UIA still verifies exact controls after native PID and root-owner checks. Before UIA access, the helper registers the matching framework's standard providers through managed reflection; direct PowerShell initialization can fail in the framework's dynamic-stack walk. Bounded scan evidence records only fixture-owned windows and controls, survives abnormal helper closure, and does not participate in target selection. Cleanup reports app Job quiescence independently from helper-tree uncertainty.
