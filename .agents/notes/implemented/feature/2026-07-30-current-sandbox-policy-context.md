...
Ownership stays narrow. Approval policy contributes its complete current `ask` or `never` fact to the same full snapshot; migrating sandbox alone would not preserve cache because `/permission` changes both owners. Plan mode remains `plan:policy`, and tool plugins continue to own schemas plus attempt, denial, and escalation guidance. The shared escalation resolver asks only when a valid target is wider than the resolved standing mode; a same or narrower target retains that standing mode without approval and never downgrades the operation. Context states standing policy; filesystem, one-shot bash, and terminal backends remain the enforcement boundaries.
