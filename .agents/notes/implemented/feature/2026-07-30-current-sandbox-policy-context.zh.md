...
归属范围仍然有限。批准策略会将其完整的当前 `ask` 或 `never` 事实贡献给同一份完整快照；只迁移沙箱无法保留缓存，因为 `/permission` 会同时改变两方。计划模式仍由 `plan:policy` 负责，工具插件也继续负责各自的 schema，以及尝试、拒绝与升权引导。共享升权解析器只在有效目标宽于解析后的常驻模式时请求审批；相同或更窄的目标会免审批保留该常驻模式，绝不会降低操作权限。上下文负责说明常驻策略；文件系统、一次性 bash 与终端后端仍是强制执行边界。
