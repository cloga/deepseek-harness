...
显式或配置的提供方、模型或强度会在提供方基线与请求优先级完成后，通过 `ctx.llm.resolveCallConfig()` 解析。具有静态路由默认值的提供方会在请求省略强度时禁止继承父级强度，从而保留所选模型的默认值。LLM 查询负责提供方注册、精确模型元数据、推理强度校验和 adapter 默认值。异步查询完成后、创建子级或后台 job 之前，工具会再次检查取消状态，并确认同一个提供方实例仍处于注册状态，因此 HMR 不会把一个提供方的默认值与另一个提供方的进程组合。既没有面向模型的选择、没有配置路由字段，也没有匹配用户自有规则的调用会保留原有提供方路径，不要求可选 LLM 服务存在。
...
随附的 `subagent_fork` 实例不会读取模型选择设置，即使进程内 fork 提供方支持 `agentOptions` 也是如此。没有匹配的[用户自有创建时规则](2026-09-19-subagent-model-rules.zh.md)时，fork 会继承父级生效的提供方与模型，使复制的对话前缀仍可供提供方侧 KV Cache 复用。更改任一路由组件都会要求新路由重新预填充继承的历史，而这项重算成本可能超过委派任务本身。该限制与发现工具的全局名称无关：分离发现工具的持有权可以让配置生效，却无法保留复用。只有在路由变化仍能保留前缀复用，或调用方可以显式限制并接受重算成本时，才重新考虑面向模型的 fork 路由选择。用户自有规则属于独立的默认设置决定，会展示这项缓存权衡，而不会授予模型额外的选择权限。
...
- 随附 fork 工具省略面向模型的路由字段。没有匹配的用户自有规则时，它们继承父级的提供方与模型，保留继承前缀的 KV Cache 复用资格。
- 省略选择时保留配置默认值和静态提供方路由默认值；否则用户自有规则可以替代来自父级的兼容继承。改变路由但不显式指定强度时，使用所选模型的默认值。
