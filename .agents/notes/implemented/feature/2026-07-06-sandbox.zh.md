...
当约束执行器被挂载时，`bash` 在完整注册 schema 中保留配对的 `sandbox_permissions` 和 `justification` 字段。请求组装只公开该会话可用的严格更宽模式，并在审批无法授权时省略两个字段；[会话感知升级 schema 决策](../bug-fix/2026-08-30-session-aware-escalation-schemas.zh.md)负责该规则。相同或更窄请求无需审批或降权，继续使用现有有效模式；更宽请求在执行前解析批准。`allowed-once` 仅将授权模式盖章到该请求上，而 `rejected`、`cancelled`、`unavailable`、缺失的 approval 服务或缺失的 agent 都以各自不同的结果文本失败关闭。授权不持久化。
...
- 完整升级字段在已挂载的执行器施加约束时存在；相同或更窄请求保持调用的有效模式且不提示任何人；没有 ApprovalService 的部署会让更宽请求失败关闭，并保持普通或非加宽调用不变。
...
- **注册的模式集合是静态的，而有效模式归每个会话所有。** 请求组装只为接收会话投影严格更宽的目标。直接提供的相同或更窄请求保留现有模式且从不提示任何人。
