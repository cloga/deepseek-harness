...
当约束执行器被挂载时，`bash` 在完整注册 schema 中保留配对的 `sandbox_permissions` 和 `justification` 字段。请求组装只公开该会话可用的严格更宽模式，并在审批无法授权时省略两个字段；[会话感知升级 schema 决策](../bug-fix/2026-08-30-session-aware-escalation-schemas.zh.md)负责该规则。执行拒绝任何不严格宽于该调用有效模式的目标。批准在执行之前解析。`allowed-once` 仅将授权模式盖章到该请求上，而 `rejected`、`cancelled`、`unavailable`、缺失的 approval 服务或缺失的 agent 都以各自不同的结果文本失败关闭。授权不持久化。
