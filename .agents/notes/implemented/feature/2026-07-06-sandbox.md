...
When a confining executor is mounted, `bash` advertises `sandbox_permissions` and `justification`. The schema accepts the full closed sandbox-mode vocabulary because effective mode is per-session. Execution rejects unknown modes, then compares the requested mode: the effective mode or a narrower mode retains the effective mode without approval, even when justification is absent or blank; a strictly wider mode requires a non-empty justification and resolves exactly one approval before execution. `allowed-once` stamps the granted mode onto only that request, while `rejected`, `cancelled`, `unavailable`, a missing approval service, or a missing agent all fail closed with distinct results. No grant is persisted.
...
- **The advertised target set is static while the effective mode is per-session** (schemas are registry-global) — a session already at the widest mode is still offered the fields. Harmless by construction: execution compares the request with the effective mode before requiring justification or approval, so non-widening requests retain the effective mode and prompt nobody; only strict widening enters the fail-closed approval path.
