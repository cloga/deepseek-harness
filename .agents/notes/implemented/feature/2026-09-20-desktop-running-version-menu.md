...
The existing About entry is relabeled with the running version through the same menu owner, without adding a duplicate entry; the native About panel remains platform-owned. Reading the version does not check for updates, download artifacts, modify settings, or restart the Host. Version display is not evidence of successful managed-update completion or plugin qualification.

Owner-local Desktop tests cover locale copy, full-version preservation, native About options and dispatch, and existing menu actions. The packaged Windows acceptance clicks the actual preload-owned Application caption control before waiting for Host readiness, on both initial startup and restart. It temporarily intercepts `Menu.prototype.popup` to inspect the resulting Electron menu model and running version, and `app.showAboutPanel` to verify the real About callback's dispatch; both methods are restored on completion or failure. Receipts record `nativePopupOpened: false` and `nativeModalOpened: false`: this verifies the caption-triggered menu model and callback dispatch, not rendered native popup or modal windows. This shell behavior has no recorded Session round trip; its expected observations belong beside Desktop tests rather than in the CLI Session snapshots. Acceptance uses only a disposable packaged application and profile, never the operator's live installation.
