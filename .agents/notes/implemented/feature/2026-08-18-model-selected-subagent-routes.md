...
Shipped `subagent_fork` instances do not read model-selection settings even though the in-process fork provider supports `agentOptions`. Without a matching [user-owned creation-time rule](2026-09-19-subagent-model-rules.md), a fork inherits the parent's effective provider and model so its copied conversation prefix remains eligible for provider-side KV Cache reuse. Changing either route component requires the new route to prefill that inherited history again, and that recomputation can dominate the delegated task's cost. This restriction is independent of the discovery tool's global name: separating discovery ownership would permit the configuration but would not preserve reuse. Model-facing fork route selection remains unavailable until a route change can retain prefix reuse or the caller can explicitly bound and accept the recomputation cost. User-owned rules are a separate default-setting decision and display this cache trade-off without granting the model additional selection authority.
...
- Shipped fork tools omit model-facing route fields. Without a matching user-owned rule they inherit the parent's provider and model, preserving eligibility for inherited-prefix KV Cache reuse.
- Omission retains configured defaults and static provider route defaults; otherwise user-owned rules may replace compatible parent inheritance. A route change without an explicit effort uses the selected model's default.
