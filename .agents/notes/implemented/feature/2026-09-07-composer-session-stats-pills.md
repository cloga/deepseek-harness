...
A new statistics-item Slot is unnecessary because the existing ordered public dock supports this placement. `display: contents` is also unnecessary: retaining the native group box preserves its geometry and avoids changing accessibility grouping. Packaged geometry acceptance opens a test-owned persisted Session through the shipped application and measures real InputBar, StatsPills, and released plugin controls; synthetic history and signed-out account state do not imply model inference or live quota access.
