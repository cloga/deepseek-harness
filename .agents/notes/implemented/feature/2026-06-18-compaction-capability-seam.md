...
`compactIfNeeded(agent, trigger, signal)` takes an explicit `'pressure' | 'context-overflow'` trigger and cancellation. It reads only the latest durable routed request; no header means no work, while any routed provider/model target uses the singleton estimator. `compactNow(agent, signal)` requires an idle agent and performs one useful balanced reduction even below pressure, returning `null` without writes when none exists. `compactRegion(start, end, agent, signal?)` uses `agent.session` as its single session identity and keeps an optional signal for explicit callers. The default summarizer resolves its target from explicit config, a manual operation's captured conversation target, the latest logged routed target, then agent options, and records the provider/model pair after any `llm/stream` routing. It replays the routed request's prefix and appends the compaction directive as a trailing user message so the provider's warm KV cache is reused — see the [`dsh-compaction-basic` README](../../../../packages/compaction/compaction-basic/README.md). The result carries `llmStreamCall: true` because it consumed exactly one call through this context's LLM service; a subclass sets that marker only under the same condition, since retained `rawOutput` alone does not identify the call path. The call sets the provider-neutral `GenerateOptions.purpose` to `compaction`; adapters may map that purpose to model-hidden transport metadata, and the DeepSeek adapter sends `x-deepseek-harness-compact: 1`.

Manual condensation captures the selected route through the Agent-scoped `model-selection/query` query after maintenance admission and before awaiting the summary. The selection owner contributes this read-only query, keeping the compaction backend independent of Web controllers. One detached target controls both the policy override and the default summary route; later selections remain pending for later work. The summary neither consumes that pending selection nor writes a conversation request header. Automatic recovery instead retains the actual durable request route: a new selection must not redirect recovery of an already-dispatched request. This distinction lets a user select another summarization model after a failed compact without first sending a conversation request that may itself overflow.

Output truncation has a distinct manual failure code. The backend still rejects the partial checkpoint, preserves transaction cleanup and cancellation precedence, and makes no hidden retry or output-budget increase. Successful-call replay remains one summary record per model call; recovery of truncated output would require separately recorded failed attempts rather than silently looping inside the summarizer.
