...
The headless defaults fixture sends an immediate SSE comment and releases its agent response only after the title request arrives. It retains exact request counts, endpoints, output, and defaults while using the adapter's ordinary idle budget; process scheduling does not decide a subsecond timeout assertion. Messages-adapter tests own a fake clock to verify comment-driven deadline renewal, successful completion beyond the original deadline, and body cancellation after silence or a post-comment stall. Production timeout and retry policies are unchanged.
