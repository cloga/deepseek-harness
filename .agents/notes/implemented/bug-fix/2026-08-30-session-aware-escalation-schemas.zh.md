...
支持沙箱的工具注册一份进程级升级 schema，但有效沙箱模式与审批策略归每个会话所有。因此，`danger-full-access` 或禁用审批的请求仍会公布 `sandbox_permissions` 和 `justification`。部分工具调用客户端要求填写所有已公布属性，导致普通命令必须携带无法请求更宽权限的升级元数据。
...
模型 schema 与拒绝提示只公开当前会话可以请求的重试。Native 与 PTC 呈现使用同一个逐 agent 投影。静态诊断保留完整模式词汇。直接或注入的相同及更窄请求无需审批或降权，继续使用现有策略；更宽请求仍接受严格校验与审批。
