...
A release plan cannot take over a conflicting manual installation. Schema-1 and strict-pin entries accept only an enabled, user-owned verified package with the identical planned source. A schema-2 entry may explicitly admit a different verified user source under the [compatible user override decision](2026-09-21-desktop-compatible-user-plugin-overrides.md); installation kinds without verified receipts and user-disabled state still require an explicit recovery operation. If an optional reconstruction fails and would remove a protected package, the whole transaction fails.

Both ordinary startup IPC and the preload-independent emergency reset path require native destructive confirmation, with cancellation selected by default, before stopping the Host or touching profile files. Cancellation restores the recovery controls; quitting or closing the owning window invalidates late acceptance. Under the held lock and after Host stop, reset verifies a private configuration/artifact copy before its first destructive write. Generated node_modules are excluded; linked configuration refuses reset. Copy failure attempts to restart the original Host, and final readiness or failure receives a separate immutable outcome.

This partially supersedes desired-name precedence and optional-failure exclusion in the [ownership decision](2026-09-17-desktop-plugin-retention-and-lockfiles.md) and [verified transaction decision](../architecture/2026-09-15-desktop-verified-release-plugin-transactions.md). Their source verification, ownership migration, lock normalization and ordinary activation rollback remain applicable. Plugin receipt, source and Session schemas remain unchanged; private activation-journal version 2 adds recovery evidence. The later compatible-override decision adds one package-specific startup recovery IPC without changing those schemas.
