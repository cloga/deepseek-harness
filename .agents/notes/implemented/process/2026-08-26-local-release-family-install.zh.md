...
该命令安装到 staging npm prefix，校验选定 checkout commit 和 DSH 版本，应用 DSH tarball payload 策略，记录每个 tarball 的 SHA-256 和文件数，并在移除 `NODE_PATH` 与 `NODE_OPTIONS` 后驱动已安装的 JavaScript entry。staged 安装必须通过 `dsh --profile web --port 0 --no-open` 报告 Web URL，之后才会替换选定 prefix。Desktop 通过 `DSH_CLI_PATH` 接收 npm 创建的 executable。
