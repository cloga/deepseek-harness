...
The command installs into a staging npm prefix, verifies the selected checkout commit and DSH version, applies the DSH tarball payload policy, records every tarball's SHA-256 and file count, and drives the installed JavaScript entry with `NODE_PATH` and `NODE_OPTIONS` removed. The staged installation must report a Web URL from `dsh --profile web --port 0 --no-open` before it replaces the selected prefix. Desktop receives the npm-created executable through `DSH_CLI_PATH`.
